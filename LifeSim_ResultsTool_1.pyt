...
# Revisions: 
    ...
    #to do: add warning and mobilization percentages
...
class Lifesim1(object):
    ...
    def execute(self, parameters, messages):
        ...
        for alternative1 in alternative_names_list:
            ...
            # create the Excel worksheet for alternative and set column widths
            ws = wb.create_sheet(title=alternative1)
            ws.column_dimensions['A'].width = 30
            ...
            try:
                ...
                raise RuntimeError(f"Error. Likely cause is that the selected Alternative was not actually computed in the selected simulation, so the following table couldn't be found: {results1_day_input}. Reason: {str(e)}")
            ...
            total_lifeloss_day_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0"
            cursor.execute(total_lifeloss_day_query,)
            total_lifeloss_day = cursor.fetchone()[0] or 0
            ...
            total_lifeloss_night_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0"
            cursor.execute(total_lifeloss_night_query,)
            total_lifeloss_night = cursor.fetchone()[0] or 0
            ...
            total_lifeloss_day_arzone1_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND {arrival_column} < ?{epzfilter}"
            cursor.execute(total_lifeloss_day_arzone1_query, (BreachTime_in_minutes,))
            total_lifeloss_day_arzone1_sum = cursor.fetchone()[0]

            total_lifeloss_day_arzone2_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND {arrival_column} > ? AND {arrival_column} < ?{epzfilter}"
            ...
            total_lifeloss_day_arzone3_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND {arrival_column} > ? AND {arrival_column} < ?{epzfilter}"
            ...
            total_lifeloss_day_arzone4_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND {arrival_column} > ? AND {arrival_column} < ?{epzfilter}"
            ...
            total_lifeloss_day_arzone5_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND {arrival_column} > ? AND {arrival_column} < ?{epzfilter}"
            ...
            total_lifeloss_day_arzone6_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND {arrival_column} > ? AND {arrival_column} < ?{epzfilter}"
            ...
            total_lifeloss_day_arzone7_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND {arrival_column} > ?{epzfilter}"
            ...
            total_lifeloss_night_arzone1_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND {arrival_column} < ?{epzfilter}"
            ...
            total_lifeloss_night_arzone2_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND {arrival_column} > ? AND {arrival_column} < ?{epzfilter}"
            ...
            total_lifeloss_night_arzone3_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND {arrival_column} > ? AND {arrival_column} < ?{epzfilter}"
            ...
            total_lifeloss_night_arzone4_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND {arrival_column} > ? AND {arrival_column} < ?{epzfilter}"
            ...
            total_lifeloss_night_arzone5_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND {arrival_column} > ? AND {arrival_column} < ?{epzfilter}"
            ...
            total_lifeloss_night_arzone6_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND {arrival_column} > ? AND {arrival_column} < ?{epzfilter}"
            ...
            total_lifeloss_night_arzone7_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND {arrival_column} > ?{epzfilter}"
            ...
            query_total_LLday_inschools = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'EDU%'"
            cursor.execute(query_total_LLday_inschools,)
            total_LLday_inschools = cursor.fetchone()[0] or 0
            query_total_LLnight_inschools = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'EDU%'"
            ...
            query_total_LLday_res1 = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'RES1%'"
            cursor.execute(query_total_LLday_res1,)
            total_LLday_res1 = cursor.fetchone()[0] or 0
            query_total_LLnight_res1 = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'RES1%'"
            ...
            query_total_LLday_res2 = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'RES2%'"
            cursor.execute(query_total_LLday_res2,)
            total_LLday_res2 = cursor.fetchone()[0] or 0
            query_total_LLnight_res2 = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'RES2%'"
            ...
            query_total_LLday_res3 = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'RES3%'"
            cursor.execute(query_total_LLday_res3,)
            total_LLday_res3 = cursor.fetchone()[0] or 0
            query_total_LLnight_res3 = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'RES3%'"
            ...
            query_total_LLday_res456 = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND Occupancy_Type IN ('RES4', 'RES5', 'RES6')"
            cursor.execute(query_total_LLday_res456,)
            total_LLday_res456 = cursor.fetchone()[0] or 0
            query_total_LLnight_res456 = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND Occupancy_Type IN ('RES4', 'RES5', 'RES6')"
            ...
            query_total_LLday_com = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'COM%'"
            cursor.execute(query_total_LLday_com,)
            total_LLday_com = cursor.fetchone()[0] or 0
            query_total_LLnight_com = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'COM%'"
            ...
            query_total_LLday_ind = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'IND%'"
            cursor.execute(query_total_LLday_ind,)
            total_LLday_ind = cursor.fetchone()[0] or 0
            query_total_LLnight_ind = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'IND%'"
            ...
            query_total_LLday_gov = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'GOV%'"
            cursor.execute(query_total_LLday_gov,)
            total_LLday_gov = cursor.fetchone()[0] or 0
            query_total_LLnight_gov = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'GOV%'"
            ...
            query_total_LLday_rel = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'REL%'"
            cursor.execute(query_total_LLday_rel,)
            total_LLday_rel = cursor.fetchone()[0] or 0
            query_total_LLnight_rel = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'REL%'"
            ...
            query_total_LLday_agr = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'AGR%'"
            cursor.execute(query_total_LLday_agr,)
            total_LLday_agr = cursor.fetchone()[0] or 0
            query_total_LLnight_agr = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND Occupancy_Type LIKE 'AGR%'"
            ...
            for col_total in range(start_col_total, start_col_total + 7):  # Adjust range as necessary
            ...
            # Apply number formatting to the relevant cells
            for col_summary in range(start_col_summary + 1, start_col_summary + 7):  # Adjust range as necessary
            ...
            #switch active worksheet back to the alternative sheet
            ws = wb[f"{alternative1}"]
            ...
            ws.cell(row=row, column=col+2, value="Struc PreHaz")
            ws.cell(row=row, column=col+3, value="Struc PostHaz")
            ws.cell(row=row, column=col+4, value="PAR PreHaz (day)")
            ws.cell(row=row, column=col+5, value="PAR PostHaz (day)")
            ws.cell(row=row, column=col+6, value="LL PreHaz (day)")
            ws.cell(row=row, column=col+7, value="LL PostHaz (day)")
            ws.cell(row=row, column=col+8, value="% Warned")
            ws.cell(row=row, column=col+9, value="% Mobilized")
            ws.cell(row=row, column=col+10, value="Fatality Rate (day)")
            ...
            for epz in epz_list:
                ...
                if epz_floodedbeforewarning:
                    ...
                    epz_floodedbeforewarning_LLday_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Emergency_Zone = ? AND Time_To_First_Wet < {epz_earliestwarning_minutes} AND Max_Depth > 0"
                    cursor.execute(epz_floodedbeforewarning_LLday_query, (epz,))
                    epz_floodedbeforewarning_LLday_sum = cursor.fetchone()[0] or 0

                    epz_floodedbeforewarning_LLnight_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Emergency_Zone = ? AND Time_To_First_Wet < {epz_earliestwarning_minutes} AND Max_Depth > 0"
                ...
                # SQL query to sum mean life loss based on pre or post hazard arrival time
                epz_posthazardarrival_LLday_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Emergency_Zone = ? AND Time_To_First_Wet > ? AND Max_Depth > 0"
                ...
                epz_prehazardarrival_LLday_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Emergency_Zone = ? AND Time_To_First_Wet < ? AND Max_Depth > 0"
                ...
                ws.cell(row=current_row_epz, column=start_col_epz+2, value=epz_prehazardarrival_day)
                ws.cell(row=current_row_epz, column=start_col_epz+3, value=epz_posthazardarrival_day)
                ws.cell(row=current_row_epz, column=start_col_epz+4, value=epz_prehazardarrival_PARday_sum)
                ws.cell(row=current_row_epz, column=start_col_epz+5, value=epz_posthazardarrival_PARday_sum)
                ws.cell(row=current_row_epz, column=start_col_epz+6, value=epz_prehazardarrival_LLday_sum or 0)
                ws.cell(row=current_row_epz, column=start_col_epz+7, value=epz_posthazardarrival_LLday_sum or 0)
                ws.cell(row=current_row_epz, column=start_col_epz+8, value=epz_par_warned_day_percentage)
                ws.cell(row=current_row_epz, column=start_col_epz+9, value=epz_par_mobilized_day_percentage)
                ws.cell(row=current_row_epz, column=start_col_epz+10, value=round(epz_fatalityrate_day, 3))

                # Apply number formatting to the relevant cells
                for col_epz in range(start_col_epz+1, start_col_epz+9):  # Adjust range as necessary
                    cell = ws.cell(row=current_row_epz, column=col_epz)
                    cell.number_format = '#,##0.0'  # Set the desired number format

                #set Percent Warned and percent mobilized to 1 decimal place %
                cell = ws.cell(row=current_row_epz, column=start_col_epz+8)
                cell.number_format = '0.0%'  # Set the desired number format    
                cell = ws.cell(row=current_row_epz, column=start_col_epz+9)
                cell.number_format = '0.0%'  # Set the desired number format
            ...
            for summarypolygonset in cleaned_list:
                ...
                for area in area_list:
                    ...
                    area_structurecount_day_query = f"SELECT COUNT(*) FROM {results1_day_input} WHERE Max_Depth > 0 AND Summary_Set_{areapolygon} = '{area}'"
                    cursor.execute(area_structurecount_day_query)
                    area_structurecount_day = cursor.fetchone()[0] or 0

                    # SQL query to count features with depth greater than zero and arrival time pre and psot hazard the numeric field above the threshold
                    area_posthazstructurecount_day_query = f"SELECT COUNT(*) FROM {results1_day_input} WHERE Time_to_First_Wet > ? AND Max_Depth > 0 AND Summary_Set_{areapolygon} = '{area}'"
                    cursor.execute(area_posthazstructurecount_day_query, (BreachTime_in_minutes,))
                    ...
                    area_avgposthazardarrival_day_query = f"SELECT AVG(Time_To_First_Wet) FROM {results1_day_input} WHERE Time_To_First_Wet > ? AND Time_To_First_Wet < 10000000 AND Max_Depth > 0 AND Summary_Set_{areapolygon} = '{area}'"
                    cursor.execute(area_avgposthazardarrival_day_query, (BreachTime_in_minutes,))
                    ...
                    area_percentile_posthazardarrival_day_query = f"""
                        ...
                        AND Max_Depth > 0 AND Summary_Set_{areapolygon} = '{area}' 
                        ORDER BY Time_To_First_Wet
                        LIMIT 1 
                        OFFSET CAST((SELECT COUNT(*) FROM {results1_day_input} 
                        WHERE Time_To_First_Wet > ? AND Time_To_First_Wet < 10000000 
                        AND Max_Depth > 0 AND Summary_Set_{areapolygon} = '{area}') * ? AS INTEGER)
                        """

                    #cursor executes with a different percentile, like 0.15 or 0.85
                    cursor.execute(area_percentile_posthazardarrival_day_query, (BreachTime_in_minutes, BreachTime_in_minutes, arrivallowpercentile))
                    ...
                    cursor.execute(area_percentile_posthazardarrival_day_query, (BreachTime_in_minutes, BreachTime_in_minutes, arrivalhighpercentile))
                    ...
                    area_percentile_depth_query = f"""
                        SELECT Max_Depth FROM {results1_day_input} 
                        WHERE Summary_Set_{areapolygon} = '{area}' 
                        AND Max_Depth > 0{epzfilter} 
                        ORDER BY Max_Depth
                        LIMIT 1 
                        OFFSET CAST((SELECT COUNT(*) FROM {results1_day_input} 
                    ...
                    cursor.execute(area_percentile_depth_query, (depthlowpercentile,))
                    area_depthlowpercent = cursor.fetchone()
                    area_depthlowpercent = round(area_depthlowpercent[0], 1) if area_depthlowpercent else 0
                    cursor.execute(area_percentile_depth_query, (depthhighpercentile,))
                    ...
                    cursor.execute(area_percentile_velocity_query, (depthlowpercentile,))
                    ...
                    cursor.execute(area_percentile_velocity_query, (depthhighpercentile,))
                    ...
                    cursor.execute(area_percentile_dxv_query, (velocitylowpercentile,))
                    area_dxvlowpercent = cursor.fetchone()
                    area_dxvlowpercent = round(area_dxvlowpercent[0], 1) if area_dxvlowpercent else 0
                    cursor.execute(area_percentile_dxv_query, (velocityhighpercentile,))
                    ...
                    fiftypercentiterations = numiterations / 2
                    query_area_collapsed50p = f"SELECT COUNT(*) FROM {results1_day_input} WHERE Max_Depth > 0 AND Summary_Set_{areapolygon} = '{area}' AND Collapsed > {fiftypercentiterations}"
                    cursor.execute(query_area_collapsed50p,)
                    ...
                    AND Max_Depth > 0 AND Summary_Set_{areapolygon} = '{area}'
                    """
                    cursor.execute(area_firstflooded_posthazardarrival_day_query, (BreachTime_in_minutes,))
                    ...
                    area_avgdepth_day_query = f"SELECT AVG(Max_Depth) FROM {results1_day_input} WHERE Max_Depth > 0 AND Summary_Set_{areapolygon} = '{area}'"
                    cursor.execute(area_avgdepth_day_query,)
                    ...
                    area_avgLL_day_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND Summary_Set_{areapolygon} = '{area}'"
                    cursor.execute(area_avgLL_day_query,)
                    ...
                    area_paru65_day_query = f"SELECT SUM(Pop_Under65_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND Summary_Set_{areapolygon} = '{area}'"
                    cursor.execute(area_paru65_day_query,)
                    area_paru65_day = cursor.fetchone()[0] or 0
                    #add_message("...Avg paru65 (day) in: {0} = {1}".format(area, round(area_paru65_day,2)), lst=2)

                    area_paro65_day_query = f"SELECT SUM(PAR_Over65_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND Summary_Set_{areapolygon} = '{area}'"
                    cursor.execute(area_paro65_day_query,)
                    ...
                    area_avgLL_night_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND Summary_Set_{areapolygon} = '{area}'"
                    cursor.execute(area_avgLL_night_query,)
                    area_avgLL_night = cursor.fetchone()[0] or 0

                    # SQL query to calculate average night PARu65 in each area
                    area_paru65_night_query = f"SELECT SUM(Pop_Under65_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND Summary_Set_{areapolygon} = '{area}'"
                    cursor.execute(area_paru65_night_query,)
                    area_paru65_night = cursor.fetchone()[0] or 0
                    #add_message("...Avg paru65 (night) in: {0} = {1}".format(area, round(area_paru65_night,2)), lst=2)
                    area_paro65_night_query = f"SELECT SUM(PAR_Over65_Mean) FROM {results1_night_input} WHERE Max_Depth > 0 AND Summary_Set_{areapolygon} = '{area}'"
                    cursor.execute(area_paro65_night_query,)
                    ...
                    area_par_warned_day_query = f"SELECT SUM(PAR_Warned_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND Summary_Set_{areapolygon} = '{area}'"
                    cursor.execute(area_par_warned_day_query,)
                    area_par_warned_day = cursor.fetchone()[0] or 0
                    area_par_warned_day_percentage = (area_par_warned_day / area_par_day) if area_par_warned_day else 0

                    # SQL query to calculate PAR_Mobilized_Mean day
                    area_par_mobilized_day_query = f"SELECT SUM(PAR_Mobilized_Mean) FROM {results1_day_input} WHERE Max_Depth > 0 AND Summary_Set_{areapolygon} = '{area}'"
                    cursor.execute(area_par_mobilized_day_query,)
                    area_par_mobilized_day = cursor.fetchone()[0] or 0
                    area_par_mobilized_day_percentage = (area_par_mobilized_day / area_par_day) if area_par_mobilized_day else 0
