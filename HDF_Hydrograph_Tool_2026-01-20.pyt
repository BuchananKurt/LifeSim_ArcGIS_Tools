        ...
        with arcpy.da.SearchCursor(jointargets, selected_fields) as cursor1:
            ...
            for row1 in cursor1:
                ...
                if hazard_time_exists:
                    ...
                    ws.cell(row=19, column=1, value=float(hazard_time1))
                    ws.cell(row=20, column=1, value=float(hazard_time2))
...
class Hdfplannames(object):
    ...
    def getParameterInfo(self):
        ...
        lifesimfolder = arcpy.Parameter(
            displayName="Check to search subfolders",
            name="Check to search subfolders",
            datatype="GPBoolean",
            parameterType="Optional",
            direction="Input")
        ...
        damsaname = arcpy.Parameter(
            displayName="Dam Storage Area Name (optional)",
        ...
        crestelev = arcpy.Parameter(
            displayName="Dam Crest Elevation (optional)",
            name="Dam Crest Elevation",
            datatype="GPString",
            parameterType="Optional", 
            direction="Input")
        #crestelev.value = "1514.0"
        
        parameters = [rasfolder, lifesimfolder, damsaname, crestelev]
        #                0           1              2             3               4                 5                  6             7
        return parameters
    ...
    def execute(self, parameters, messages):
        ...
        storage_area_crest_elev = parameters[3].valueAsText

        if storage_area_crest_elev is not None:
            storage_area_crest_elev = float(storage_area_crest_elev)
        ...
        column_mod = 0
        #adds columns for storage area data if a storage area is selected
        if get_storage_volume and storage_area_name is not None and get_file_path:
            column_mod = 6
        elif get_storage_volume and storage_area_name is not None:
            column_mod = 4
        elif get_file_path:
            column_mod = 2
        ...
        if get_file_path:
            ws.cell(row=1, column=3, value="Original HDF Path")
            ws.cell(row=1, column=4, value="Current HDF Folder")
        ...
        for file in hdf_files:
            ...
            with h5py.File(file, 'r') as hdf_file:
                ...
                messages.addMessage(f"...READING HDF FILE: {file_name}")
                
                # Reset interval and time variables for each file
                interval_num = None
                interval_unit = None
                parsed_plan_start = None
                ...
                if plan_info_table is not None:
                    ...
                    if get_file_path:
                        hdf_path = plan_info_table.attrs.get('Project Filename', 'N/A')
                        ws.cell(row=currentrow, column=3, value=hdf_path)
                        plan_folder = os.path.basename(os.path.dirname(file))
                        ws.cell(row=currentrow, column=4, value=plan_folder)
                        
                    
                    # Extract Simulation Start Time and parse it
                    plan_starttime_value = plan_info_table.attrs.get('Simulation Start Time', 'N/A')
                    if isinstance(plan_starttime_value, bytes):
                        plan_starttime_value = plan_starttime_value.decode('ascii', errors='ignore').rstrip('\x00').strip()
                    if plan_starttime_value != 'N/A':
                        try:
                            parsed_plan_start = datetime.strptime(plan_starttime_value, "%d%b%Y %H:%M:%S")
                        except (ValueError, TypeError):
                            parsed_plan_start = None
                    
                    # Extract Base Output Interval and parse it
                    plan_outputinterval_value = plan_info_table.attrs.get('Base Output Interval', 'N/A')
                    if isinstance(plan_outputinterval_value, bytes):
                        plan_outputinterval_value = plan_outputinterval_value.decode('ascii', errors='ignore').rstrip('\x00').strip()
                    if plan_outputinterval_value != 'N/A':
                        match = re.match(r"(\d+)([A-Za-z]+)", plan_outputinterval_value)
                        if match:
                            interval_num = match.group(1)   # "15" or "10" or "1"
                            interval_unit = match.group(2)  # "MIN" or "HOUR"
                    ...
                    messages.addMessage(f"Valid information found for plan: {plan_number}, name: {plan_shortid}")
                ...
                if get_storage_volume and storage_area_name is not None:
                    ...
                    if sa_variable_table_key in hdf_file:
                        if get_file_path:
                            ...
                            startstage_col = 3
                        stage_col = startstage_col + 1 #4
                        vol_col = startstage_col + 2 #5
                        ot_time_col = startstage_col + 3 #6
                        ...
                        ws.cell(row=1, column=ot_time_col, value="OT Time")
                        if storage_area_crest_elev is not None:
                            ws.cell(row=1, column=ot_time_col, value=f"OT Time ({storage_area_crest_elev} ft)")
                        ...
                        if sa_variable_table is not None:
                            ...
                            # Identify the row where the stage first exceeds the crest elevation
                            if storage_area_crest_elev is not None:
                                exceed_indices = np.where(sa_stage_values > storage_area_crest_elev)[0]
                                # Identify the time value corresponding to the first exceedance, if it exists
                                if exceed_indices.size > 0:
                                    first_exceed_index = exceed_indices[0]
                                    # Calculate the timestamp for this row based on Simulation Start Time and Base Output Interval
                                    if interval_num and interval_unit and parsed_plan_start:
                                        # Convert interval_num to integer
                                        interval_num_int = int(interval_num)
                                        # Create a timedelta based on interval_unit
                                        if interval_unit == 'MIN':
                                            interval_delta = timedelta(minutes=interval_num_int)
                                        elif interval_unit == 'HOUR':
                                            interval_delta = timedelta(hours=interval_num_int)
                                        elif interval_unit == 'DAY':
                                            interval_delta = timedelta(days=interval_num_int)
                                        else:
                                            interval_delta = timedelta(minutes=interval_num_int)  # Default to minutes
                                        
                                        # Calculate the timestamp for the first exceedance
                                        exceed_time = parsed_plan_start + (first_exceed_index * interval_delta)
                                        messages.addMessage(f"Storage Area '{storage_area_name}' exceeds crest elevation at row {first_exceed_index}, time: {exceed_time.strftime('%d%b%Y %H:%M:%S')}")
                                        cell=ws.cell(row=currentrow, column=ot_time_col, value=exceed_time)
                                    else:
                                        messages.addMessage(f"Storage Area '{storage_area_name}' exceeds crest elevation at row index {first_exceed_index}, but unable to calculate timestamp.")
                                        cell=ws.cell(row=currentrow, column=ot_time_col, value="UNK")
                                else:
                                    messages.addMessage(f"Storage Area '{storage_area_name}' stage does not exceed crest elevation of {storage_area_crest_elev} ft.")
                                    cell=ws.cell(row=currentrow, column=ot_time_col, value="No OT")
                            else:
                                messages.addMessage(f"Storage Area '{storage_area_name}' crest elevation not provided; skipping OT time calculation.")
                                cell=ws.cell(row=currentrow, column=ot_time_col, value="N/A")
                ...
                if get_breach_data:
                    ...
                    if breach_data_table_key in hdf_file:
                        ...
                        for item in breach_data_name_values:
                            ...
                            # QC check brst contains everything after the first bar, with bars removed except those originally in the string
        ...
        output_excel = f'{rasfolder}/~RAS_Plan_Data.xlsx'
        # Save the workbook
