...
#FY25-C 7/25/2025: added check in NSI tool to flag if the output is over 2 gigs
#FY25-D 8/19/2025: added NLD levee download tool
#FY25-E 9/5/2025: added option to not dissolve study area in polygons to double warning, allowing it to split study area features into breach and nonbreach zones
#FY25-E 10/2/2025: NSI API tool reprojects to study area projection by default
#FY26-A 12/2/2025: Added tool to make DamCat on an inventory exported from LifeSim, updated index to FY26 1.216 from Oct25 CCI index of 14073.99
#FY26-B 3/3/2026: changed NSI local shapefile tool to look for county shapefiles within subfolders so you don't have to have all files in one folder, it can be state level folders
    # compatible with NSI geopackages or shapefiles. Also a new tool that joins LifeSim results to a polygon
#FY26-C 6/25/2026: added ability to strip unnecessary fields from NSI from local files tool
#FY26-E 8/12/2026: added selection for FIPS field in NSI tools and fixed the skip for indexing if index is 1.
#FY26-F 8/25/2026: added ability to strip fields prior to merge in the NSI from API tool for large areas
#FY26-G 9/3/2026: added ability to simplify nonbreach polygons in the Polygons to Double Warning tool, fixed buffer/simplify
#-------------------------------------------------------------------------
...
class Toolbox(object):
    def __init__(self):
        ...
        self.tools = [Nsicreator, Bufferandsimplifypoly, Polygonstodoublewarning, Nsicreatorapi, Reclassifytozone, Rastertopolygon, Snaptoexclusionpoly, Batchdefineprojection, Incrementallifesimresults, Nldleveedownload, occtyptodamcat, lifesimresultstopolygon]
...
class Nsicreator(object):
    def __init__(self):
        ...
        self.label = "NSI Creator (from local shp or gpkg)"
    ...
    def getParameterInfo(self):
        ...
        inputuscounties = arcpy.Parameter(
            displayName="Counties Shapefile w/ FIPS field",
            name="us counties shapefile",
            datatype="GPFeatureLayer",
            parameterType="Required",
            direction="Input")
        #dropdown field list of fields in uscounties shapefile to use for FIPS field, default is FIPS
        fipsfieldname = arcpy.Parameter(
            displayName="FIPS Field in Counties Shapefile",
            name="fips field",
            datatype="GPString",
            parameterType="Required",
            direction="Input")
        inputnsifolder = arcpy.Parameter(
            displayName="Folder with NSI Files",
        ...
        addfianame = arcpy.Parameter(
            ...
            direction="Input")
        stripfields = arcpy.Parameter(
            displayName="Check to strip unnecessary fields (NSI-2026 only, can help keep shp dbf under 2GB limit)",
            name="check to strip fields",
            datatype="GPBoolean",
            parameterType="Optional",
            direction="Input")
        
        parameters = [inputstudyarea, workspace, inputuscounties, fipsfieldname, inputnsifolder, priceindex, outputname, addfianame, stripfields]
        #                      0         1          2                 3                4             5           6            7              8
        return parameters
    ...
    def updateParameters(self, parameters):
        ...
        has been changed."""
        #populates dropdown list of field names for uscounties shapefile to use for FIPS field
        if parameters[2].valueAsText:  # Check if the counties shapefile parameter has a value
            field_names = [f.name for f in arcpy.ListFields(parameters[2].valueAsText)]
            parameters[3].filter.list = field_names  # Set the filter list for the FIPS field parameter
    ...
    def execute(self, parameters, messages):
        ...
        fipsfield = parameters[3].valueAsText
        inputnsifolder = parameters[4].valueAsText
        priceindex = parameters[5].valueAsText
        outputname = parameters[6].valueAsText
        #boolean parameters should just be the value, not text
        addfianame = parameters[7].value
        stripfields = parameters[8].value
        ...
        #fipsfield = "GEOID"
        messages.addMessage("FIPS field is: {0}".format(fipsfield))

        #create list of all the FIPS county numbers in the clipped shapefile
        FipsList = [row[0] for row in arcpy.da.SearchCursor(ClippedUSCounties, [fipsfield])]
        ...
        for FIPS_Value in FipsList:
            ...
            # Search the input NSI folder and all subfolders for a shapefile named {FIPS_Value}.shp
            shp_matches = glob.glob(os.path.join(inputnsifolder, "**", f"{FIPS_Value}.shp"), recursive=True)
            
            # Search for geopackage named {FIPS_Value}.gpkg
            gpkg_matches = glob.glob(os.path.join(inputnsifolder, "**", f"{FIPS_Value}.gpkg"), recursive=True)
            
            if shp_matches:
                FIPS_Value_features = shp_matches[0]
                messages.addMessage(f"{FIPS_Value}.shp found in NSI folder or subfolder")
                #nsi2022 = True
            elif gpkg_matches:
                # Construct path to the "points" feature class inside the geopackage
                FIPS_Value_features = os.path.join(gpkg_matches[0], f"{FIPS_Value}")
                messages.addMessage(f"{FIPS_Value}.gpkg\\{FIPS_Value} found in NSI folder or subfolder")
                #nsi2026 = True
            else:
                messages.addWarningMessage(f"No shapefile or geopackage found for FIPS {FIPS_Value} in {inputnsifolder} or subfolders. Skipping.")
                continue
            ...
            arcpy.analysis.PairwiseClip(
                in_features=FIPS_Value_features, 
                clip_features=inputstudyarea, 
            ...
            if stripfields:
                ...
                fields_to_keep = ["FID", "Shape", "OCCTYPE", "ST_DAMCAT", "POP2AMU65", "POP2AMO65", "POP2PMU65", "POP2PMO65", "NUM_STORY", "X", "Y", "FOUND_HT", "EXTWALL", "VAL_STRUCT", "VAL_CONT", "VAL_VEHIC"]
                fields_to_keep_upper = [f.upper() for f in fields_to_keep]
                # Get a list of all fields in the feature class
                all_fields = [f.name for f in arcpy.ListFields(clippednsicounties)]
                # Determine which fields to delete, comparison is case-insensitive
                fields_to_delete = [f for f in all_fields if f.upper() not in fields_to_keep_upper]
                # Delete the unnecessary fields
                arcpy.management.DeleteField(clippednsicounties, fields_to_delete)
                messages.addMessage(f"Stripped unnecessary fields from {clippednsicounties}")
        ...
        #check shapefile size and warn if approaching 2GB limit
        import glob
        shapefile_base = os.path.splitext(os.path.basename(outputnsifile))[0]
        dbf_path = os.path.join(workspace, shapefile_base + ".dbf")
        total_size = os.path.getsize(dbf_path) if os.path.exists(dbf_path) else 0

        # 2 GB = 2 * 1024 * 1024 * 1024 bytes
        if total_size > 1900000000:  # 2 GB in bytes
            messages.addWarningMessage(
                f"WARNING: The output shapefile '{shapefile_base}' is approaching ({total_size/1024/1024/1024:.2f} GB). "
                ...
                "Retry tool with the option checked to strip unecessary fields to reduce shapefile size"
                )
        ...
        # Default to 1.0 if empty, otherwise convert
        if not priceindex:
            priceindex = 1.0
        else:
            try:
                ...
                messages.addErrorMessage("Price index must be a numeric value. Please enter a valid number.")
                return
        #index value fields using calculate field, unless the index is 1.0
        priceindex = float(priceindex)
        if priceindex == 1.0:
            messages.addMessage("Price index is 1.0, no indexing will be applied to structure value, content value, or vehicle value")
        else:
            ...
            arcpy.management.CalculateField(in_table=outputnsifile, 
                field="Val_Struct", 
                expression=f"round(!Val_Struct! * {priceindex})", 
            ...
            arcpy.management.CalculateField(in_table=outputnsifile, 
                field="Val_Cont", 
                expression=f"round(!Val_Cont! * {priceindex})", 
                expression_type="PYTHON_9.3", 
                code_block="", 
                field_type="TEXT", 
                enforce_domains="NO_ENFORCE_DOMAINS")
            ...
            arcpy.management.CalculateField(in_table=outputnsifile, 
                field="Val_Vehic", 
                expression=f"round(!Val_Vehic! * {priceindex})", 
                expression_type="PYTHON_9.3", 
                code_block="", 
                field_type="TEXT", 
                enforce_domains="NO_ENFORCE_DOMAINS")
        ...
        if addfianame:
            ...
            try:
                ...
                messages.addErrorMessage(f"Error adding or calculating ST_Name field, try manually doing it: {e}")
...
class Bufferandsimplifypoly(object):
    ...
    def getParameterInfo(self):
        ...
        bufferdistance = arcpy.Parameter(
            ...
            parameterType="Required",
            direction="Input")
        simplifytolerance = arcpy.Parameter(
            displayName="Simplify Tolerance in Feet (optional, if not specified it will be buffer distance divided by 4)",
            name="simplify_tolerance",
            datatype="GPLong",
            parameterType="Optional",
            direction="Input")
        parameters = [inputpolygon, outputpolygon, bufferdistance, simplifytolerance]
        return parameters
    ...
    def execute(self, parameters, messages):
        ...
        simplifytolerance = parameters[3].valueAsText

        # find folder location of outputpolygon
        output_folder = os.path.dirname(outputpolygon)
        interimpolygon = os.path.join(output_folder, "bufferedpoly.shp")
        dissolved_output = os.path.join(output_folder, "dissolvedpoly.shp")
        ...
        messages.addMessage(f"Buffered polygon, saved to {interimpolygon}")

        if simplifytolerance:
            simplifytolerance = str(int(simplifytolerance)) +" Feet"
        else:
            simplifytolerance = str(int(bufferdistance) // 4) +" Feet"
        ...
        messages.addMessage(f"Deleted interim buffered polygon from {interimpolygon}")
...
class Polygonstodoublewarning(object):
    ...
    def getParameterInfo(self):
        ...
        scenario2name = arcpy.Parameter(
            ...
            parameterType="Optional",
            direction="Input")
        dissolvestudyarea = arcpy.Parameter(
            displayName="Check to dissolve study area first (default)",
            name="Check to dissolve study area first",
            datatype="GPBoolean",
            parameterType="Optional",
            direction="Input")
        dissolvestudyarea.value = True
        studyareanamefield = arcpy.Parameter(
            displayName="Field in Study Area to use for naming (only if not dissolving)",
            name="field in study area for naming",
            datatype="GPString",
            parameterType="Optional",
            direction="Input")
        studyareanamefield.filter.type = "ValueList"
        studyareanamefield.filter.list = []  # Will be populated in updateParameters
        simplifynb = arcpy.Parameter(
                    displayName="Check to simplify non-fail area w/ 10-ft tolerance (default)",
                    name="Check to simplify non-fail area",
                    datatype="GPBoolean",
                    parameterType="Optional",
                    direction="Input")
        simplifynb.value = True
        ...
        info_text.value = "Typically used to create EPZs with a breach and nonbreach zone. " \
        "\nIf you do not dissolve the study area first, you could have a study area with additional zones. " \
        "\nUse case for additional zones would be areas with different warning/mob curves, like an elicitation. " \
        "\nIf not dissolving, check results, sometimes extra slivers will need to be deleted or merged."

        parameters = [inputstudyarea, inputinpoolpolygon, inputnonbreach1, epzfolder, scenario1name, inputnonbreach2, scenario2name, dissolvestudyarea, studyareanamefield, simplifynb, info_text]
        return parameters
    ...
    def updateParameters(self, parameters):
        ...
        has been changed."""
        inputstudyarea = parameters[0].valueAsText
        studyareanamefield_param = parameters[8]  # Adjust index if needed

        if inputstudyarea:
            # List all fields in the input study area
            field_names = [f.name for f in arcpy.ListFields(inputstudyarea)]
            studyareanamefield_param.filter.list = field_names
    ...
    def execute(self, parameters, messages):
        ...
        dissolve = parameters[7].value
        studyareanamefield = parameters[8].valueAsText
        simplifynb = parameters[9].value
        ...
        nonbreach1simp = fr"{epzfolder}\nonbreach1simp.shp"
        nonbreach2simp= fr"{epzfolder}\nonbreach2simp.shp"


        #dissolve = False
        #studyareanamefield = "zname"
        ...
        if dissolve:
            ...
            arcpy.management.AddField(in_table=studyareadissolve, field_name="Fname", 
                field_type="TEXT", field_precision=None, field_scale=None, field_length=30, field_alias="", 
        ...
        if dissolve == False:
            arcpy.management.CopyFeatures(inputstudyarea, studyareadissolve)
            messages.addMessage("Copied Study Area to interim folder")
            ...
            # Process: Calculate Field Fname Study Area (Calculate Field) (management)
            expression1 = f"!{studyareanamefield}!"
            arcpy.management.CalculateField(in_table=studyareadissolve, field="Fname", 
                expression=expression1, expression_type="PYTHON_9.3", code_block="", 
                field_type="TEXT", enforce_domains="NO_ENFORCE_DOMAINS")[0]
            messages.addMessage("Calculated Fname field in Study Area")
        ...
        if simplifynb:
            # Process: Simplify NF-1 (Simplify Polygon) (cartography)
            arcpy.cartography.SimplifyPolygon(in_features=nonbreach1dissolve, 
                out_feature_class=nonbreach1simp, algorithm="POINT_REMOVE", 
                tolerance="10 Feet", minimum_area="0 SquareFeet", error_option="RESOLVE_ERRORS", 
                in_barriers=[])
            messages.addMessage("Simplified NF-1")
            nonbreach1dissolve = nonbreach1simp
        ...
        arcpy.management.AddField(in_table=nonbreach1dissolve, field_name="NFname", field_type="TEXT", 
            field_precision=None, field_scale=None, field_length=30, field_alias="", field_is_nullable="NULLABLE", field_is_required="NON_REQUIRED", field_domain="")[0]
        ...
        arcpy.management.AddField(in_table=scenario1union, field_name="Zone", field_type="TEXT", field_precision=None, 
            field_scale=None, field_length=30, field_alias="", field_is_nullable="NULLABLE", field_is_required="NON_REQUIRED", field_domain="")[0]
        ...
        if dissolve == False:
            arcpy.management.CalculateField(in_table=scenario1union, field="Zone", 
                ...
                if ipname == 'InPool_EPZ':
                    ...
                    return "Non_Breach_EPZ_" + str(fname)
                else:
                    return "Breach_EPZ_" + str(fname)""", field_type="TEXT", enforce_domains="NO_ENFORCE_DOMAINS")[0]
        ...
        if inputnonbreach2:
            ...
            if simplifynb:
                # Process: Simplify NF-2 (Simplify Polygon) (cartography)
                arcpy.cartography.SimplifyPolygon(in_features=nonbreach2dissolve, 
                    out_feature_class=nonbreach2simp, algorithm="POINT_REMOVE", 
                    tolerance="10 Feet", minimum_area="0 SquareFeet", error_option="RESOLVE_ERRORS", 
                    collapsed_point_option="NO_KEEP", in_barriers=[])
                messages.addMessage("Simplified Optional NF-2")
                nonbreach2dissolve = nonbreach2simp
            ...
            if simplifynb:
                arcpy.management.Delete(fr"{epzfolder}\nonbreach2dissolve.shp")
                arcpy.management.Delete(fr"{epzfolder}\nonbreach2simp_Pnt.shp")
...
class Nsicreatorapi(object):
    ...
    def getParameterInfo(self):
        ...
        workspace = arcpy.Parameter(
            ...
            direction="Input")
        inputuscounties = arcpy.Parameter(
            displayName="US Counties Shapefile w/ FIPS field",
            name="us counties shapefile",
            datatype="GPFeatureLayer",
            parameterType="Required",
            direction="Input")
        ...
        reprojectoutput = arcpy.Parameter(
            displayName="Check to set output projection to study area projection",
            name="Check to set output projection to study area projection",
            datatype="GPBoolean",
            parameterType="Optional",
            direction="Input")
        reprojectoutput.value = True
        stripfields = arcpy.Parameter(
                    displayName="Check to strip unnecessary fields (Can help keep shp dbf under 2GB limit)",
                    name="check to strip fields",
                    datatype="GPBoolean",
                    parameterType="Optional",
                    direction="Input")

        parameters = [inputstudyarea, workspace, inputuscounties, fipsfieldname, priceindex, outputname, addfianame, internalnsi, splitbypolygons, clearbuffer, reprojectoutput, stripfields]
    ...
    def execute(self, parameters, messages):
        ...
        internalnsi = parameters[7].value
        splitbypolygons = parameters[8].value
        clearbuffer = parameters[9].value
        reproject = parameters[10].value
        stripfields = parameters[11].value
        #clearbuffer = True
        ...
        for FIPS_Value in FipsList:
            ...
            if stripfields:
                #list of fields to keep
                fields_to_keep = ["FID", "OBJECTID", "Shape", "OCCTYPE", "ST_DAMCAT", "POP2AMU65", "POP2AMO65", "POP2PMU65", "POP2PMO65", "NUM_STORY", "X", "Y", "FOUND_HT", "BLDGTYPE", "VAL_STRUCT", "VAL_CONT", "VAL_VEHIC"]
                fields_to_keep_upper = [f.upper() for f in fields_to_keep]
                # Get a list of all fields in the feature class
                all_fields = [f.name for f in arcpy.ListFields(clippedcountygpkgtable)]
                # Determine which fields to delete, comparison is case-insensitive
                fields_to_delete = [f for f in all_fields if f.upper() not in fields_to_keep_upper]
                # Delete the unnecessary fields
                arcpy.management.DeleteField(clippedcountygpkgtable, fields_to_delete)
                messages.addMessage(f"Stripped unnecessary fields from {clippedcountygpkgtable}")
        ...
        if priceindex == 1.0:
            ...
            messages.addMessage("Indexing structure values, content values, and vehicle values by {0}...".format(priceindex))
            arcpy.management.CalculateField(in_table=outputnsifile, 
                field="Val_Struct", 
        ...
        if addfianame:
            ...
            try:
                ...
                messages.addWarningMessage(f"Adding or calculating ST_Name field failed: {e}")
                messages.addWarningMessage("Try manually adding a field named ST_Name and calculating it as OccType + FID in the output shapefile or geopackage.")
        ...
        if reproject:
            ...
            # Reproject the output to match the spatial reference of the input study area
            try:
                desc = arcpy.Describe(inputstudyarea)
                target_sr = desc.spatialReference
                if not target_sr or target_sr.name == "Unknown":
                    messages.addWarningMessage("Input study area has unknown spatial reference; defaulting to Alers Standard.")
                    target_sr = usa_albers_spatial_ref
                messages.addMessage(f"Reprojecting the output file to match input study area ({target_sr.name})...")
                arcpy.management.Project(outputnsifile, outputnsifileprj, target_sr)
                # delete original output and continue with reprojected file
                arcpy.management.Delete(outputnsifile)
                outputnsifile = outputnsifileprj
            except Exception as e:
                messages.addWarningMessage(f"Reprojection to input study area spatial reference failed: {e}")
...
class Nldleveedownload(object):
    def __init__(self):
        """Define the tool (tool name is the name of the class)."""
        self.label = "Download NLD Levees from API"
        self.description = ""
        self.canRunInBackground = False
    ...
    def getParameterInfo(self):
        ...
        outputpolygon = arcpy.Parameter(
            displayName = "Output Levee Polygon Name",
            name = "Output Levee Polygon Name",
            datatype = "GPFeatureLayer",
            parameterType = "Required",
            direction = "Output",)
        
        parameters = [inputpolygon, outputpolygon]
        return parameters
    ...
    def execute(self, parameters, messages):
        ...
        # --- NLD REST service link, levee area polygons are feature 17, link current as of 8/19/2025---
        nld_levee_layer_url = "https://geospatial.sec.usace.army.mil/dls/rest/services/NLD/Public/FeatureServer/17"

        # --- WGS84 Spatial Reference ---
        wgs84 = arcpy.SpatialReference(4326)

        query_url = f"{nld_levee_layer_url}/query"
        ...
        # --- Step 1: Check projection of input polygon ---
        desc = arcpy.Describe(input_polygon)
        input_sr = desc.spatialReference

        messages.addMessage(f"Input spatial reference: {input_sr.name}")

        if input_sr.factoryCode != 4326:
            messages.addMessage("Input not in WGS84. Reprojecting...")
            input_polygon_wgs84 = os.path.join(arcpy.env.scratchGDB, "input_polygon_wgs84")
            arcpy.management.Project(input_polygon, input_polygon_wgs84, wgs84)
        else:
            messages.addMessage("Input already in WGS84.")
            input_polygon_wgs84 = input_polygon

        # --- Step 2: Get extent of (possibly reprojected) polygon ---
        extent = arcpy.Describe(input_polygon_wgs84).extent
        envelope = f"{extent.XMin},{extent.YMin},{extent.XMax},{extent.YMax}"
        messages.addMessage(f"Using extent: {envelope}")

        # Add geometry filter to params
        params.update({
            "geometry": envelope,
        ...
        messages.addMessage(f"Querying NLD service...")
        response = requests.get(query_url, params=params)

        if response.status_code != 200:
            raise Exception(f"Failed to query NLD service: {response.status_code} - {response.text}")

        geojson = response.json()

        if "features" not in geojson or len(geojson["features"]) == 0:
            messages.addMessage("No levee areas found in the specified extent.")
        else:
            messages.addMessage(f"Retrieved {len(geojson['features'])} levee area(s).")

        # --- Step 4: Save to temp GeoJSON ---
        temp_geojson = os.path.join(arcpy.env.scratchFolder, "levee_areas_temp.geojson")
        with open(temp_geojson, "w") as f:
            json.dump(geojson, f)

        # --- Step 5: Convert to feature class ---
        if input_sr.factoryCode != 4326:
            output_fc_temp = os.path.splitext(output_fc)[0] + "_temp.shp"
            arcpy.conversion.JSONToFeatures(temp_geojson, output_fc_temp)
            messages.addMessage(f"WGS84 Levee areas saved to: {output_fc_temp}")
        else:
            arcpy.conversion.JSONToFeatures(temp_geojson, output_fc)
            messages.addMessage(f"[SUCCESS] Levee areas saved to: {output_fc}")

        # --- Optional: Reproject output to match input polygon's original spatial reference ---
        if input_sr.factoryCode != 4326:
            messages.addMessage(f"Reprojecting output to match input polygon's spatial reference: {input_sr.name}")
            arcpy.management.Project(output_fc_temp, output_fc, input_sr)
            messages.addMessage(f"[SUCCESS] Reprojected levee output saved to: {output_fc}")

        # --- Cleanup intermediate files ---
        try:
            if os.path.exists(temp_geojson):
                os.remove(temp_geojson)
                messages.addMessage(f"Deleted temporary geojson file: {temp_geojson}")
        except Exception as e:
            messages.addMessage(f"[WARNING] Could not delete temporary geojson file: {temp_geojson} ({e})")

        if input_sr.factoryCode != 4326:
            try:
                if arcpy.Exists(input_polygon_wgs84):
                    arcpy.Delete_management(input_polygon_wgs84)
                    messages.addMessage(f"Deleted intermediate wgs84 polygon: {input_polygon_wgs84}")
            except Exception as e:
                messages.addMessage(f"[WARNING] Could not delete intermediate wgs84 polygon: {input_polygon_wgs84} ({e})")
            try:
                if arcpy.Exists(output_fc_temp):
                    arcpy.Delete_management(output_fc_temp)
                    messages.addMessage(f"Deleted intermediate wgs84 levees: {output_fc_temp}")
            except Exception as e:
                messages.addMessage(f"[WARNING] Could not delete intermediate wgs84 levees: {output_fc_temp} ({e})")

        return
#---------------------------------------------------------------

class occtyptodamcat(object):
    def __init__(self):
        """Define the tool (tool name is the name of the class)."""
        self.label = "Create DamCats from Occ Types"
        self.description = ""
        self.canRunInBackground = True

    def getParameterInfo(self):
        """Define parameter definitions"""
        inputinventory = arcpy.Parameter(
            displayName="Input Inventory Exported from LifeSim",
            name="input_polygon",
            datatype="GPFeatureLayer",
            parameterType="Required",
            direction="Input")
        occtypenamefield = arcpy.Parameter(
            displayName="Occupancy Type Field in Input Inventory",
            name="Occupancy Type Field in Input Inventory",
            datatype="GPString",
            parameterType="Required",
            direction="Input")
        occtypenamefield.filter.type = "ValueList"
        occtypenamefield.filter.list = []  # Will be populated in updateParameters
        
        parameters = [inputinventory, occtypenamefield]
        return parameters
    ...
    def updateParameters(self, parameters):
        ...
        validation is performed.  This method is called whenever a parameter
        has been changed."""
        inputinventory = parameters[0].valueAsText
        occtypenamefield_param = parameters[1]  # Adjust index if needed

        if inputinventory:
            # List all fields in the input study area
            field_names = [f.name for f in arcpy.ListFields(inputinventory)]
            occtypenamefield_param.filter.list = field_names
        return
    ...
    def execute(self, parameters, messages):
        """The source code of the tool."""
        #call parameters that were entered and create new variables as needed
        input_inventory = parameters[0].valueAsText
        occtype_field = parameters[1].valueAsText
        
        damcat_field = "DamCat"

        # Add the new field
        arcpy.management.AddField(
            in_table=input_inventory,
            field_name=damcat_field,
            field_type="TEXT",
            field_length=15
        )
        
        ## begin damcat calculation
        messages.addMessage("Calculating damage categories...")
        damcatExpression = fr"Damcat(str(!{occtype_field}!))"
        damcat_codeblock = """def Damcat(occtyp):
            if occtyp == "RES1-1SNB" or occtyp == "RES1-1SWB" or occtyp == "RES1-2SNB" or occtyp == "RES1-2SWB" or occtyp == "RES1-3SNB" or occtyp == "RES1-3SWB" or occtyp == "RES1-SLNB" or occtyp == "RES1-SLWB" or occtyp == "RES2" or occtyp == "RES3A" or occtyp == "RES3B" or occtyp == "RES3C" or occtyp == "RES3D" or occtyp == "RES3E" or occtyp == "RES3F" or occtyp == "RES4" or occtyp == "RES5" or occtyp == "RES6":
                return "RES"
            elif occtyp == "COM1" or occtyp == "COM2" or occtyp == "COM3" or occtyp == "COM4" or occtyp == "COM5" or occtyp == "COM6" or occtyp == "COM7" or occtyp == "COM8" or occtyp == "COM9" or occtyp == "COM10" or occtyp == "AGR1" or occtyp == "REL1":
                return "COM"
            elif occtyp == "IND1" or occtyp == "IND2" or occtyp == "IND3" or occtyp == "IND4" or occtyp == "IND5" or occtyp == "IND6":
                return "IND"
            elif occtyp == "GOV1" or occtyp == "GOV2" or occtyp == "EDU1" or occtyp == "EDU2":
                return "PUB"
            else:
                return "Unk" """

        arcpy.management.CalculateField(input_inventory, damcat_field, damcatExpression, "PYTHON3", damcat_codeblock)
...
##-------------------------------------------------------------------------------------
class lifesimresultstopolygon(object):
    def __init__(self):
        """Define the tool (tool name is the name of the class)."""
        self.label = "LifeSim Results to Polygons"
        self.description = ""
        self.canRunInBackground = True

    def getParameterInfo(self):
        """Define parameter definitions"""
        inputresults = arcpy.Parameter(
            displayName="Input Results from LifeSim",
            name="input_results",
            datatype="GPFeatureLayer",
            parameterType="Required",
            direction="Input")
        inputpolygons = arcpy.Parameter(
            displayName="Input Polygons",
            name="input_polygons",
            datatype="GPFeatureLayer",
            parameterType="Required",
            direction="Input")
        #polygonnamefield = arcpy.Parameter(
            #displayName="Name Field in Polygons",
            #name="polygon_name_field",
            #datatype="GPString",
            #parameterType="Required",
            #direction="Input")
        #polygonnamefield.filter.type = "ValueList"
        #polygonnamefield.filter.list = []  # Will be populated in updateParameters
        
        outputpolygons = arcpy.Parameter(
            displayName="Output Polygons with Results",
            name="output_polygons",
            datatype="GPFeatureLayer",
            parameterType="Required",
            direction="Output")
        
        #parameters = [inputresults, inputpolygons, polygonnamefield, outputpolygons]
        parameters = [inputresults, inputpolygons, outputpolygons]
        return parameters
    ...
    def updateParameters(self, parameters):
        ...
        validation is performed.  This method is called whenever a parameter
        has been changed."""
        #inputpolygons = parameters[1].valueAsText
        #polygonnamefield_param = parameters[2]  # Adjust index if needed

        #if inputpolygons:
            # List all fields in the input study area
            #field_names = [f.name for f in arcpy.ListFields(inputpolygons)]
            #polygonnamefield_param.filter.list = field_names
        return
    ...
    def execute(self, parameters, messages):
        """The source code of the tool."""
        #call parameters that were entered and create new variables as needed
        input_results = parameters[0].valueAsText
        input_polygons = parameters[1].valueAsText
        #polygonnamefield = parameters[2].valueAsText
        output_polygons = parameters[2].valueAsText

        # Add a helper field for counting points
        messages.addMessage("Adding point count helper field...")
        if not any(f.name == "Num_Strucs" for f in arcpy.ListFields(input_results)):
            arcpy.management.AddField(input_results, "Num_Strucs", "SHORT")
            arcpy.management.CalculateField(input_results, "Num_Strucs", "1", "PYTHON3")
            messages.addMessage("Num_Strucs field added and calculated.")
        else:
            messages.addMessage("Num_Strucs field already exists.")

        # Define which fields from input_results to keep and how to aggregate them
        # Modify the field names and merge rules as needed
        fields_config = {
            "Num_Strucs": "Sum",             # Points per polygon
            "Pop_Under7": "Sum",      # Change field_name_1 to actual field and Sum/First/Mean/etc
            "PAR_Over65": "Sum",
            "Life_Loss9": "Sum",      # Add or remove fields as needed
            "Structure1": "Sum",
            "Content__1": "Sum",
            "Vehicle__1": "Sum"
        }
        ...
        # Add all fields from the target polygons
        field_mapping.addTable(input_polygons)
        
        # Get all fields from input_results
        results_field_map = arcpy.FieldMappings()
        results_field_map.addTable(input_results)
        
        # Keep only specified fields from input_results and set merge rules
        for i in range(len(results_field_map.fields) - 1, -1, -1):
            field = results_field_map.fields[i]
            # Remove field if not in fields_config and not OBJECTID
            if field.name not in fields_config.keys() and field.name != "OBJECTID":
                results_field_map.removeFieldMap(i)
        
        # Apply merge rules to the fields we're keeping
        for i in range(len(results_field_map.fields)):
            field_map = results_field_map.getFieldMap(i)
            field_name = results_field_map.fields[i].name
            if field_name in fields_config:
                field_map.mergeRule = fields_config[field_name]
                results_field_map.replaceFieldMap(i, field_map)
        
        # Add the filtered results fields to main field mapping
        for i in range(len(results_field_map.fields)):
            field_map = results_field_map.getFieldMap(i)
            field_mapping.addFieldMap(field_map)

        # Use the Spatial Join tool to join the results points to the polygons
        messages.addMessage("Performing spatial join...")
        arcpy.analysis.SpatialJoin(
            target_features=input_polygons, 
            join_features=input_results, 
            out_feature_class=output_polygons, 
            join_operation="JOIN_ONE_TO_ONE", 
            match_option="INTERSECT",
            field_mapping=field_mapping)

        messages.addMessage("Spatial join completed. Adding calculated sum fields...")

        # Add field for PAR (Population At Risk) sum
        arcpy.management.AddField(output_polygons, "PAR_Total", "DOUBLE")
        arcpy.management.CalculateField(
            output_polygons, 
            "PAR_Total", 
            "!Pop_Under7! + !PAR_Over65!", 
            "PYTHON3"
        )
        messages.addMessage("Added PAR_Total field (Pop_Under7 + PAR_Over65)")

        # Add field for Total Damage sum
        arcpy.management.AddField(output_polygons, "Total_Dmg", "DOUBLE")
        arcpy.management.CalculateField(
            output_polygons, 
            "Total_Dmg", 
            "!Structure1! + !Content__1! + !Vehicle__1!", 
            "PYTHON3"
        )
        messages.addMessage("Added Total_Dmg field (Structure1 + Content__1 + Vehicle__1)")
