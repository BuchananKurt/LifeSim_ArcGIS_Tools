        ...
        for alternative1 in alternative_names_list:
            ...
            for time_label, results_table, total_lifeloss in time_table_list:
                ...
                if top10_rows and len(top10_rows[0]) == 12:
                    headers = ["Rank", "FID", "Occupancy Type", "Number Stories", "EPZ", "Summary Area", "Collapsed", "PAR", "Max Depth", "Max Velocity", "Arrival Time hrs", "LL Evac", "Total LL", "% of Total LL"]
                else:
                    headers = ["Rank", "FID", "Max Depth", "Total Life Loss", "% of Total LL"]
                ...
                for time_label, results_table, total_road_lifeloss, road_network_name in time_table_roads_list:
                    ...
                    try:
                        for row in top10_road_rows:
                            fid = row[0]
                            road_query = f"""
                                SELECT highway, name, "tiger:county", bridge
                            ...
                            if info:
                                highway, road_name, county, bridge = info
                            else:
                                highway = road_name = county = bridge = ''
                            # normalize None to empty string
                            road_info_map[fid] = (road_name or '', highway or '', county or '', bridge or '')
                    ...
                    if top10_road_rows and len(top10_road_rows[0]) == 8:
                        headers = ["Rank", "FID", "CFCC", "One Way", "Vert Offset", "Max Depth", "Max Velocity", "Arrival Time hrs", "Life Loss", "% of Road LL", "Road Name", "Highway Type", "County", "Bridge"]
                    else:
                        headers = ["Rank", "FID", "CFCC", "Life Loss", "% of Road LL", "Road Name", "Highway Type", "County"]
                    for c, h in enumerate(headers):
                        cell = ws.cell(row=header_row, column=start_col_total + c, value=h)
                        cell.alignment = Alignment(horizontal='center')
                        cell.font = Font(bold=True)
                    ...
                    for i, rowvals in enumerate(top10_road_rows, start=1):
                        ...
                        if len(rowvals) == 8:
                            ...
                            road_name, highway_type, county, bridge = road_info_map.get(fid, ('', '', '', ''))
                            cell = ws.cell(row=out_row, column=start_col_total+1, value=fid)
