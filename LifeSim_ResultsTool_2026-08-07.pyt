...
# Created:     Sep 2024
# ArcGIS Version:   2.9.3 (but using 3.4.2 as of 2026-03-26)
...
# Revisions: 
    ...
    #2025-09-22 added capability to include life loss on road networks, analyzes road networks for vertical offsets, reports top 10 life loss structures
    #2025-11-04 added top 10 roads with life loss if evac is ran, added RAS information from hdf file
    #2025-11-18 added workaround for running a sim with only 2am or only 2pm time of day, calculates day and night as same value
    #2025-12-23 checks each alternative for convergence
    #2026-03-17 added EPZ sampling correlation reports. Also made compatible with LifeSim 2.0 from before a Collapsed result field was added.
    #2026-03-26 added check to hazard and breach times, added structure details for highest fatality rate structures
    #2026-04-09 added another time format for older 2024ish models without the am/pm
    #2026-08-07 reorganized summary are table fields, removed area filter from lack of use
#-------------------------------------------------------------------------
import arcpy
import os, math
...
class Toolbox(object):
    def __init__(self):
        ...
        .pyt file)."""
        self.label = "LifeSim Results Toolbox"
        self.alias = "LifeSim Results Toolbox"
...
def calculate_correlations(df, field_pairs, add_message_func):
    """
    Calculate Pearson correlations between specified field pairs.
    
    Args:
        df: DataFrame containing the data
        field_pairs: List of tuples [(field1, field2), ...] for correlation pairs
        add_message_func: Function to call for logging (add_message)
    ...
    for field1, field2 in field_pairs:
        ...
        if field1 not in df.columns or field2 not in df.columns:
            add_message_func(f"...Warning: Field '{field1}' or '{field2}' not found in data", lst=2)
            correlation_results[(field1, field2)] = {"correlation": None, "p_value": None, "n": 0}
            continue
        
        # Extract valid (non-null) pairs
        valid_data = df[[field1, field2]].dropna()
        
        if len(valid_data) < 3:
            add_message_func(f"...Insufficient data for correlation between {field1} and {field2} (n={len(valid_data)})", lst=2)
            correlation_results[(field1, field2)] = {"correlation": None, "p_value": None, "n": len(valid_data)}
            continue
        
        try:
            # Calculate Pearson correlation
            corr_coef, p_value = stats.pearsonr(valid_data[field1], valid_data[field2])
            correlation_results[(field1, field2)] = {
            ...
            #add_message_func(f"...Correlation {field1} vs {field2}: r={corr_coef:.4f}, p={p_value:.4f} {sig_indicator} (n={len(valid_data)})", lst=2)
            
        except Exception as e:
            add_message_func(f"...Error calculating correlation for {field1} vs {field2}: {str(e)}", lst=2)
            correlation_results[(field1, field2)] = {"correlation": None, "p_value": None, "n": 0}
...
class Lifesim1(object):
    ...
    def getParameterInfo(self):
        ...
        inputlifesimfile = arcpy.Parameter(
            ...
            direction="Input")
        inputlifesimfile.filter.list = ["fia"]   # Only show .fia files
        ...
        exportresults = arcpy.Parameter(
            ...
            parameterType="Optional",
            direction="Input")
        alphabetize_areas = arcpy.Parameter(
            displayName="Check to alphabetize area names in output",
            name="Check to alphabetize area names in output",
            datatype="GPBoolean",
            parameterType="Optional",
            direction="Input")

        parameters = [inputlifesimfile, simulation1, arrival_option, range_option, output_excel_file, alternative1, mmc_sop, exportresults, alphabetize_areas]
        #                0                  1              2             3               4                 5           7          8               9
        return parameters
    ...
    def execute(self, parameters, messages):
        ...
        mmc_sop = parameters[6].value
        export = parameters[7].value
        alphabetize_areas = parameters[8].value
        ...
        conv_tol=0.05
        min_iters=20
        #define default life loss columns for EPZ convergence testing
        DEFAULT_LL_COLUMNS: List[str] = [
            "Total_Life_Loss",
        ]

        # sets convergence and parameter correlation testing to day or night
        detailed_check_time_of_day = "Day" # options are "Day" or Night"
        if detailed_check_time_of_day == "Day":
            detail_time = 14
        else:
            detail_time = 2
        ...
        alt_to_sheet_map = []
        # helper function to add messages as both normal messages and append them to the lists
        ...
        # define reusable Excel fonts so they are available everywhere
        green_font = Font(color="008000")
        #orange_font = Font(color="FFA500")
        orange_font = Font(color="D68A00")
        red_font = Font(color="C80000")
        ...
        def table_header(worksheet, start_row, start_col, headers):
            for col_offset, header in enumerate(headers):
                cell = worksheet.cell(row=start_row, column=start_col + col_offset, value=header)
                cell.alignment = Alignment(horizontal='center')
                cell.font = Font(bold=True)

        # helper function that does convergence testing
        def convergence_by_ci(df, ll_columns, conf_level, conv_tol, min_iters, ddof=1):
            """
            Reproduce Excel-style convergence test.
            - df: DataFrame with per-iteration columns (rows = iterations)
            - ll_columns: list of columns to sum into per-iteration total
            - conf_level: confidence level (0.95 -> z ≈ 1.96)
            - conv_tol: relative tolerance (e.g., 0.05 for 5%)
            - min_iters: minimum iterations before declaring convergence
            - ddof: degrees of freedom for std (1 for sample std)
            Returns DataFrame with cum_mean, cum_std, margin, error_pct and convergence info.
            """

            totals = df[ll_columns].sum(axis=1).reset_index(drop=True).astype(float)
            # If all totals are exactly zero, treat as already converged (avoid NaNs / divide-by-zero)
            if len(totals) > 0 and (totals == 0).all():
                n = np.arange(1, len(totals) + 1)
                cum_mean = totals.expanding(min_periods=1).mean()
                cum_std = totals.expanding(min_periods=1).std(ddof=ddof).fillna(0.0)
                margin = pd.Series(0.0, index=cum_mean.index)
                error_pct = pd.Series(0.0, index=cum_mean.index)
            ...
            # z for common confidence levels (fallback mapping)
            z_map = {0.90: 1.6448536269514722, 0.95: 1.95996398454005, 0.99: 2.5758293035489004}
            #z = z_map.get(round(conf_level, 8))
            z = z_map.get(conf_level)
            if z is None:
                try:
                    from scipy.stats import norm
                    z = norm.ppf(1 - (1 - conf_level) / 2)
                except Exception:
                    # fallback to 95% if unknown and scipy not available
                    z = 1.95996398454005

            margin = z * cum_std / np.sqrt(n)
            error_pct = margin / cum_mean.replace(0, np.nan).abs()
            ...
            # first iteration that meets error_pct < conv_tol and n >= min_iters
            valid = (result["error_pct"] < conv_tol) & (result["n"] >= min_iters)
            converged_at = int(result.index[valid].tolist()[0]) if valid.any() else None
            result.attrs.update({
                "converged": converged_at is not None,
                "converged_at": converged_at,
                "final_error_pct": float(result["error_pct"].iat[-1]) if not pd.isna(result["error_pct"].iat[-1]) else None,
                "z": z,
                "conv_tol": conv_tol,
                "conf_level": conf_level
            })
        ...
        #add file name of this python script
        script_path = os.path.abspath(__file__)
        add_message("Script Version Name:   {}".format(os.path.basename(script_path)), lst=1)
        ...
        add_message("Covergence/Correlation day or night:   {}".format(detailed_check_time_of_day), lst=1)
        add_message(f'Convergence Tolerance: {conv_tol}, Confidence Level: {conf_level}', lst=1)
        add_message("Simulation Name:   {}".format(simulation1), lst=2)

        # column widths on Summary page
        column_widths = {
            'A': 30,
            'B': 60,
            'C': 20,
        ...
        table_header(ws, row, col, ["Alternative", "Structure #", "PAR Day", "PAR Night", "LL Day", "LL Night", "Damage Total", "", "LL Evac Day", "LL Evac Night"])
        ...
        # get list of times of day ran in simulation
        sim_time_fields = ["Time_0", "Time_2", "Time_4", "Time_6", "Time_8", "Time_10", "Time_12", "Time_14", "Time_16", "Time_18", "Time_20", "Time_22"]
        fields_sql = ", ".join([f'"{f}"' for f in sim_time_fields])
        cursor.execute(f"SELECT {fields_sql} FROM Simulations_Lookup_Table WHERE Name = ?;", (simulation1,))
        row = cursor.fetchone()
        if row:
            # treat 1, '1', True, 'True' as active
            sim_times_active = [name for name, val in zip(sim_time_fields, row) if val in (1, '1', True, 'True')]
        else:
            sim_times_active = []
        times_of_day = []
        times_of_day_night_only = False
        times_of_day_day_only = False
        if sim_times_active == ['Time_2', 'Time_14']:
            times_of_day == [2,14]
            times_of_day_readable = ["Night", "Day"]
        elif sim_times_active == ['Time_2']:
            times_of_day = [2]
            times_of_day_readable = ["Night"]
            times_of_day_night_only = True
            add_message(f"WARNING!!!!! Only NIGHT time was simulated, output results for day and night are actually both night.", lst=1)
        elif sim_times_active == ['Time_14']:
            times_of_day = [14]
            times_of_day_readable = ["Day"]
            times_of_day_day_only = True
            add_message(f"WARNING!!!!! Only DAY time was simulated, output results for day and night are actually both day.", lst=1)
        else:
            times_of_day = []
            times_of_day_readable = []
            add_message(f"WARNING: Unhandled combination of simulation time fields active: {sim_times_active}. Results may be incomplete.", lst=1)
        add_message(f"Times of day in simulation: {times_of_day_readable}", lst=1)

        # Query unique Road_Network names from Alternatives_Lookup_Table, check for vertical offsets and bridges
        placeholders = ','.join(['?'] * len(alternative_names_list))
        rdquery = f"SELECT DISTINCT Road_Network FROM Alternatives_Lookup_Table WHERE Road_Network IS NOT NULL AND Name IN ({placeholders});"
        cursor.execute(rdquery, alternative_names_list)
        ...
        if not unique_road_networks:
            ...
            add_message(f"None of the Alternatives have a road network selected.", lst=1)
        ...
        # set to "" because this are dropdown was used due to lack of use
        areafile = ""
        if areafile:
        ...
        add_message(f"", lst=1)
        # sort the cleaned_list alphabetically (case-insensitive) before area loops
        if alphabetize_areas:
            cleaned_list.sort(key=lambda s: s.lower())
        ...
        for alternative1 in alternative_names_list:
            ...
            # convergence flag for this alternative (init)
            convergence_test_failed = False
            
            # checks for anything that makes this look like a non-fail scenario
            #if any(s in alternative1.lower() for s in ("nb", "nf", "nonbreach", "non-breach", "nofail", "nonfail", "non-fail")):
            if any(s in alternative1.lower() for s in ("nb", "nf", "nonbreach", "non breach", "non-breach", "non_breach", "nonfail", "non-fail", "non_fail", "nofail", "no fail", "no breach", "nonfailure", "non-failure", "non_failure")):
            ...
            # adjust if only a single time of day is ran
            if times_of_day_day_only:
                results1_night_input = fr'"{simulation1}>Structure_Summary>{alternative1}>14"'
            if times_of_day_night_only:
                results1_day_input = fr'"{simulation1}>Structure_Summary>{alternative1}>2"'
            ...
            # adjust if only a single time of day is ran
            if times_of_day_day_only:
                results1_night_roads = fr'"{simulation1}>Roads_Summary>{alternative1}>14"'
            if times_of_day_night_only:
                results1_day_roads = fr'"{simulation1}>Roads_Summary>{alternative1}>2"'
            ...
            hydvertunit_query = "SELECT Vertical_Units FROM Hydraulic_Data_Lookup_Table WHERE Name = ?;"
            cursor.execute(hydvertunit_query, (hydscenario,))
            hydvertunit = cursor.fetchone()[0]

            # HDF Plan Data: try-except block attempts to access HDF5 file and read attributes
            try:
                hdf_path = fr'{outputfolder1}\\Hydraulic_Data\\{hydscenario}\\{hydscenario}.hdf'
                ...
                add_message(f"RAS Version: {ras_fileversion}", lst=2)

                # Access HDF5 dataset Event Conditions table
                hdf_eventconditionstable = hdf_file.get(f'Event Conditions')
                if hdf_eventconditionstable is not None:
                    # Get the attribute directly, Your HDF5 Event Conditions is a group with attributes (not a dataset or table).Each attribute is a key-value pair, and the value is a bytes string.
                    plan_processdate_value = hdf_eventconditionstable.attrs.get('Date Processed')
                    plan_processdate_value = plan_processdate_value.decode('ascii', errors='ignore').rstrip('\x00').strip()
                add_message(f"RAS Date Processed/Ran: {plan_processdate_value}", lst=2)

                #get last modified date of HDF file, should be when it was imported into LifeSim
                hdf_mod_time = os.path.getmtime(hdf_path)
                hdf_mod_datetime = datetime.fromtimestamp(hdf_mod_time)
                add_message(f"Imported to LifeSim on: {hdf_mod_datetime.strftime('%#m/%#d/%Y %H:%M:%S')}", lst=2)

                # Access HDF5 dataset Plan Information table
                ...
                add_message(f"RAS Plan Filename: {plan_filename_value}", lst=2)
                add_message(f"RAS Plan ShortID: {plan_shortid_value}", lst=2)
                add_message(f"RAS Plan Flow Name: {plan_flowtitle_value}", lst=2)

                # Access HDF5 dataset Geometry table
                hdf_geometrytable = hdf_file.get(f'Geometry')
                if hdf_geometrytable is not None:
                    plan_terrain_value = hdf_geometrytable.attrs.get('Terrain Filename')
                    plan_terrain_value = plan_terrain_value.decode('ascii', errors='ignore').rstrip('\x00').strip()
                    plan_siunits_value = hdf_geometrytable.attrs.get('SI Units')
                    plan_siunits_value = plan_siunits_value.decode('ascii', errors='ignore').rstrip('\x00').strip()
                add_message(f"RAS Terrain Filename: {plan_terrain_value}", lst=2)
                add_message(f"RAS SI Units?: {plan_siunits_value}", lst=2)
                
                # Warn if possible unit mismatches between LifeSim setting and RAS data
                if hydvertunit == "Feet" and plan_siunits_value is "True":
                    add_message(f"WARNING: LifeSim hydraulic vertical units are in Feet but RAS Plan indicates SI Units. Possible unit mismatch.", lst=2)
                if hydvertunit == "Meters" and plan_siunits_value is "False":
                    add_message(f"WARNING: LifeSim Hydraulic vertical units are in Meters but RAS Plan indicates it is not SI units. Possible unit mismatch.", lst=2)

                # Check for "Breaching Variables" tables in HDF5 file and pull "Breach at" time if it exists, which is when the model predicts the breach actually occurs (as opposed to just imminent hazard time)
                breach_paths = []
                base_output_path = 'Results/Unsteady/Output/Output Blocks/Base Output'
                if base_output_path in hdf_file:
                    base_group = hdf_file[base_output_path]
                    def find_breaching_variables(group, path='', results=None):
                        if results is None:
                            results = []
                        for key in group.keys():
                            item = group[key]
                            current_path = f"{path}/{key}" if path else key
                            if key in ('Breaching Variables', 'Breach Variables'):
                                results.append(current_path)
                            if isinstance(item, h5py.Group):
                                find_breaching_variables(item, current_path, results)
                        return results
                    
                    breach_paths = find_breaching_variables(base_group, base_output_path)
                
                if breach_paths:
                    for breach_path in breach_paths:
                        hdf_breachingvariablestable = hdf_file.get(breach_path)
                        if hdf_breachingvariablestable is not None:
                            breach_time_value = hdf_breachingvariablestable.attrs.get('Breach at')
                            path_parts = breach_path.split('/')
                            parent_name = path_parts[-2] if len(path_parts) > 1 else 'Unknown'
                            if breach_time_value is not None:
                                breach_time_value = breach_time_value.decode('ascii', errors='ignore').rstrip('\x00').strip()
                                # Try to reformat the date/time from "04FEB2099 18:45:00" to "2099-02-04 18:45:00"
                                try:
                                    breach_time_formatted = datetime.strptime(breach_time_value, "%d%b%Y %H:%M:%S").strftime("%Y-%m-%d %H:%M:%S")
                                except (ValueError, TypeError):
                                    breach_time_formatted = breach_time_value
                                add_message(f"RAS Breach Time for {parent_name}: {breach_time_formatted}", lst=2)
                            else:
                                add_message(f"RAS Breach Time attribute not found in Breaching Variables table for {parent_name}.", lst=2)
                else:
                    add_message(f"No RAS Breaching Variables table found in HDF.", lst=2)
            
            except Exception as e:
                add_message(f"WARNING: Could not read HDF5 file for hydraulic scenario {hydscenario}: {e}", lst=2)
            ...
            def parse_date(date_string):
                ...
                formats = [
                    ...
                    "%m/%d/%Y %I:%M %p",     # Format with AM/PM notation for LifeSim 2.0
                    "%m/%d/%Y %H:%M"         # Format with 24-hour time, no AM/PM
                ]
            ...
            add_message("Hydraulic Vertical Units:   {}".format(hydvertunit), lst=2)

            #Check to see if imminent Hazard time is the same as one of the breach times in the HDF5 file if any exist and if the HDF file can be read
            try:
                if breach_paths and not breach_condition == "NonFail":
                    breach_time_matches = []
                    for breach_path in breach_paths:
                        hdf_breachingvariablestable = hdf_file.get(breach_path)
                        if hdf_breachingvariablestable is not None:
                            ...
                            if breach_time_value is not None:
                                ...
                                try:
                                    ...
                                    breach_time_formatted = breach_time_value
                                if breach_time_formatted == haztime_formatted.strftime("%Y-%m-%d %H:%M:%S"):
                                    breach_time_matches.append((breach_path, breach_time_formatted))
                    if breach_time_matches:
                        for match in breach_time_matches:
                            add_message(f"RAS Breach Time and LifeSim Imminent Hazard Time match: {match[0]} with time {match[1]}", lst=2)
                    else:
                        # add message with the difference in hours and minutes  between the breach time that is closest to the imminent hazard time and the imminent hazard time itself, if any breach times exist
                        if breach_paths:
                            closest_breach_time = None
                            ...
                            for breach_path in breach_paths:
                                ...
                                if hdf_breachingvariablestable is not None:
                                    ...
                                    if breach_time_value is not None:
                                        breach_time_value = breach_time_value.decode('ascii', errors='ignore').rstrip('\x00').strip()
                                        try:
                                            breach_time_formatted = datetime.strptime(breach_time_value, "%d%b%Y %H:%M:%S")
                                        except (ValueError, TypeError):
                                            continue
                                        time_diff = abs((breach_time_formatted - haztime_formatted).total_seconds())
                                        if smallest_time_diff is None or time_diff < smallest_time_diff:
                                            smallest_time_diff = time_diff
                                            closest_breach_time = breach_time_formatted
                            if closest_breach_time is not None:
                                time_diff_hours = int(smallest_time_diff // 3600)
                                time_diff_minutes = int((smallest_time_diff % 3600) // 60)
                                add_message(f"Possible Issue: Imminent Hazard Time doesn't match closest breach: {closest_breach_time.strftime('%Y-%m-%d %H:%M:%S')} (Difference: {time_diff_hours} hours, {time_diff_minutes} minutes)", lst=2)
                            else:
                                add_message("Possible Issue: No match found between any RAS Breach Times and LifeSim Imminent Hazard Time.", lst=2)
            except Exception as e:
                add_message(f"WARNING: Could not read HDF5 file for hydraulic scenario {hydscenario}: {e}", lst=2)
            ...
            posthazardarrival_LLday_query = f"SELECT SUM(Life_Loss_Total_Mean + Life_Loss_Evacuating_Mean) FROM {results1_day_input} WHERE {arrival_column} > ? AND Max_Depth > 0"
            ...
            prehazardarrival_LLday_query = f"SELECT SUM(Life_Loss_Total_Mean + Life_Loss_Evacuating_Mean) FROM {results1_day_input} WHERE {arrival_column} < ? AND Max_Depth > 0"
            ...
            nohazardarrival_LLday_query = f"SELECT SUM(Life_Loss_Total_Mean + Life_Loss_Evacuating_Mean) FROM {results1_day_input} WHERE Max_Depth = 0"
            cursor.execute(nohazardarrival_LLday_query)
            nohazardarrival_LLday_sum = cursor.fetchone()[0] or 0
            if nohazardarrival_LLday_sum > 0:
                add_message("LL from Structures with no flooding (day):   {}".format(round(nohazardarrival_LLday_sum,2)), lst=2)
            ...
            query_total_LLday_inschools = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_day_input} WHERE Occupancy_Type LIKE 'EDU%'"
            cursor.execute(query_total_LLday_inschools,)
            total_LLday_inschools = cursor.fetchone()[0] or 0
            query_total_LLnight_inschools = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_night_input} WHERE Occupancy_Type LIKE 'EDU%'"
            ...
            query_total_LLday_res1 = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_day_input} WHERE Occupancy_Type LIKE 'RES1%'"
            cursor.execute(query_total_LLday_res1,)
            total_LLday_res1 = cursor.fetchone()[0] or 0
            query_total_LLnight_res1 = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_night_input} WHERE Occupancy_Type LIKE 'RES1%'"
            ...
            query_total_LLday_res2 = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_day_input} WHERE Occupancy_Type LIKE 'RES2%'"
            cursor.execute(query_total_LLday_res2,)
            total_LLday_res2 = cursor.fetchone()[0] or 0
            query_total_LLnight_res2 = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_night_input} WHERE Occupancy_Type LIKE 'RES2%'"
            ...
            query_total_LLday_res3 = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_day_input} WHERE Occupancy_Type LIKE 'RES3%'"
            cursor.execute(query_total_LLday_res3,)
            total_LLday_res3 = cursor.fetchone()[0] or 0
            query_total_LLnight_res3 = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_night_input} WHERE Occupancy_Type LIKE 'RES3%'"
            ...
            query_total_LLday_res456 = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_day_input} WHERE Occupancy_Type IN ('RES4', 'RES5', 'RES6')"
            cursor.execute(query_total_LLday_res456,)
            total_LLday_res456 = cursor.fetchone()[0] or 0
            query_total_LLnight_res456 = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_night_input} WHERE Occupancy_Type IN ('RES4', 'RES5', 'RES6')"
            ...
            query_total_LLday_com = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_day_input} WHERE Occupancy_Type LIKE 'COM%'"
            cursor.execute(query_total_LLday_com,)
            total_LLday_com = cursor.fetchone()[0] or 0
            query_total_LLnight_com = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_night_input} WHERE Occupancy_Type LIKE 'COM%'"
            ...
            query_total_LLday_ind = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_day_input} WHERE Occupancy_Type LIKE 'IND%'"
            cursor.execute(query_total_LLday_ind,)
            total_LLday_ind = cursor.fetchone()[0] or 0
            query_total_LLnight_ind = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_night_input} WHERE Occupancy_Type LIKE 'IND%'"
            ...
            query_total_LLday_gov = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_day_input} WHERE Occupancy_Type LIKE 'GOV%'"
            cursor.execute(query_total_LLday_gov,)
            total_LLday_gov = cursor.fetchone()[0] or 0
            query_total_LLnight_gov = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_night_input} WHERE Occupancy_Type LIKE 'GOV%'"
            ...
            query_total_LLday_rel = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_day_input} WHERE Occupancy_Type LIKE 'REL%'"
            cursor.execute(query_total_LLday_rel,)
            total_LLday_rel = cursor.fetchone()[0] or 0
            query_total_LLnight_rel = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_night_input} WHERE Occupancy_Type LIKE 'REL%'"
            ...
            query_total_LLday_agr = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_day_input} WHERE Occupancy_Type LIKE 'AGR%'"
            cursor.execute(query_total_LLday_agr,)
            total_LLday_agr = cursor.fetchone()[0] or 0
            query_total_LLnight_agr = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_night_input} WHERE Occupancy_Type LIKE 'AGR%'"
            ...
            # Define first_summary_set for use in fatality rate queries
            summary_set_list = []
            for summarypolygonset in cleaned_list:
                ...
                summary_set_list.append(f"Summary_Set_{areapolygon}")
            
            first_summary_set = summary_set_list[0] if summary_set_list else None
            ...
            if max_fatality_day_row is not None:
                #add_message(f"FID of structure with highest fatality rate (day):   {max_fatality_day_row[0]}", lst=2)
                ...
                # Query to get all structures with the maximum fatality rate with detailed information
                summary_set_fields = ", ".join(summary_set_list)
                query_maxfatalityday_details = f"SELECT FID, Occupancy_Type, Emergency_Zone, {summary_set_fields}, (Pop_Under65_Mean + PAR_Over65_Mean) AS Total_PAR, (Life_Loss_Total_Mean + Life_Loss_Evacuating_Mean) AS Total_Life_Loss FROM {results1_day_input} WHERE Max_Depth > 0 AND (Pop_Under65_Mean + PAR_Over65_Mean) > ? AND (Life_Loss_Total_Mean / NULLIF((Pop_Under65_Mean + PAR_Over65_Mean), 0)) = ?"
                cursor.execute(query_maxfatalityday_details, (ir_par_threshold, total_maxfatality_day,))
                maxfatalityday_details = cursor.fetchall()
                total_countmaxfatalityday = len(maxfatalityday_details) or 0
                add_message("Structures with the max fatality rate (day):   {:,.0f}".format(total_countmaxfatalityday), lst=2)
                
                # Display detailed information for each structure with maximum fatality rate
                for structure in maxfatalityday_details:
                    fid = structure[0]
                    occ_type = structure[1]
                    epz = structure[2]
                    num_summary_sets = len(summary_set_list)
                    summary_areas = structure[3:3+num_summary_sets]
                    total_par = structure[3+num_summary_sets]
                    total_ll = structure[3+num_summary_sets+1]
                    
                    # Format summary areas with field names (remove 'Summary_Set_' prefix)
                    summary_area_str = " | ".join([f"{summary_set_list[i].replace('Summary_Set_', '')}: {area}" for i, area in enumerate(summary_areas)])
                    
                    add_message(f"  FID: {fid} | Occ: {occ_type} | EPZ: {epz} | {summary_area_str} | PAR: {total_par:.1f} | Life Loss: {total_ll:.1f}  | Fatality Rate: {total_maxfatality_day:.3f}", lst=2)
            ...
            if max_fatality_night_row is not None:
                ...
                # Query to get all structures with the maximum fatality rate with detailed information
                query_maxfatalitynight_details = f"SELECT FID, Occupancy_Type, Emergency_Zone, {summary_set_fields}, (Pop_Under65_Mean + PAR_Over65_Mean) AS Total_PAR, (Life_Loss_Total_Mean + Life_Loss_Evacuating_Mean) AS Total_Life_Loss FROM {results1_night_input} WHERE Max_Depth > 0 AND (Pop_Under65_Mean + PAR_Over65_Mean) > ? AND (Life_Loss_Total_Mean / NULLIF((Pop_Under65_Mean + PAR_Over65_Mean), 0)) = ?"
                cursor.execute(query_maxfatalitynight_details, (ir_par_threshold, total_maxfatality_night,))
                maxfatalitynight_details = cursor.fetchall()
                total_countmaxfatalitynight = len(maxfatalitynight_details) or 0
                add_message("Structures with the max fatality rate (night):   {:,.0f}".format(total_countmaxfatalitynight), lst=2)
                
                # Display detailed information for each structure with maximum fatality rate
                for structure in maxfatalitynight_details:
                    fid = structure[0]
                    occ_type = structure[1]
                    epz = structure[2]
                    num_summary_sets = len(summary_set_list)
                    ...
                    add_message(f"  FID: {fid} | Occupancy: {occ_type} | EPZ: {epz} | {summary_area_str} | PAR: {total_par:.1f} | Life Loss: {total_ll:.1f} | Fatality Rate: {total_maxfatality_night:.3f}", lst=2)
            ...
            query_total_over90fatality_day = f"SELECT COUNT(*) FROM {results1_day_input} WHERE (Pop_Under65_Mean + PAR_Over65_Mean) > ? AND (Life_Loss_Total_Mean / NULLIF((Pop_Under65_Mean + PAR_Over65_Mean), 0)) > 0.9"
            ...
            # # of structs with over 90% fatality rate
            query_total_over90fatality_night = f"SELECT COUNT(*) FROM {results1_night_input} WHERE (Pop_Under65_Mean + PAR_Over65_Mean) > ? AND (Life_Loss_Total_Mean / NULLIF((Pop_Under65_Mean + PAR_Over65_Mean), 0)) > 0.9"
            ...
            col = start_col_total
            # Set headers for columns in total table
            table_header(ws, row, col, ["Alternative", "Structure #", "PAR Day", "PAR Night", "LL Day", "LL Night", "Damage Total"])

            if evacuation_simulated:
                table_header(ws,row, col+7, ["LL Evac Day", "LL Evac Night"])
            ...
            table_header(ws, current_row_total, col, ["Arrival Zone", "Structure #", "PAR Day", "PAR Night", "LL Day", "LL Night", "Depth Range", "Velocity Range", "% Warned (Day)", "% Mobilized (Day)", "Fatality Rate (Day)"])
            ...
            cell = ws.cell(row=current_row_total, column=start_col_total+6, value=arzone1_depth_range)
            cell.alignment = Alignment(horizontal='center')
            cell = ws.cell(row=current_row_total, column=start_col_total+7, value=arzone1_velocity_range)
            cell.alignment = Alignment(horizontal='center')
            ...
            cell = ws.cell(row=current_row_total, column=start_col_total+6, value=arzone2_depth_range)
            cell.alignment = Alignment(horizontal='center')
            cell = ws.cell(row=current_row_total, column=start_col_total+7, value=arzone2_velocity_range)
            cell.alignment = Alignment(horizontal='center')
            ...
            cell = ws.cell(row=current_row_total, column=start_col_total+6, value=arzone3_depth_range)
            cell.alignment = Alignment(horizontal='center')
            cell = ws.cell(row=current_row_total, column=start_col_total+7, value=arzone3_velocity_range)
            cell.alignment = Alignment(horizontal='center')
            ...
            cell = ws.cell(row=current_row_total, column=start_col_total+6, value=arzone4_depth_range)
            cell.alignment = Alignment(horizontal='center')
            cell = ws.cell(row=current_row_total, column=start_col_total+7, value=arzone4_velocity_range)
            cell.alignment = Alignment(horizontal='center')
            ...
            cell = ws.cell(row=current_row_total, column=start_col_total+6, value=arzone5_depth_range)
            cell.alignment = Alignment(horizontal='center')
            cell = ws.cell(row=current_row_total, column=start_col_total+7, value=arzone5_velocity_range)
            cell.alignment = Alignment(horizontal='center')
            ...
            cell = ws.cell(row=current_row_total, column=start_col_total+6, value=arzone6_depth_range)
            cell.alignment = Alignment(horizontal='center')
            cell = ws.cell(row=current_row_total, column=start_col_total+7, value=arzone6_velocity_range)
            cell.alignment = Alignment(horizontal='center')
            ...
            cell = ws.cell(row=current_row_total, column=start_col_total+6, value=arzone7_depth_range)
            cell.alignment = Alignment(horizontal='center')
            cell = ws.cell(row=current_row_total, column=start_col_total+7, value=arzone7_velocity_range)
            cell.alignment = Alignment(horizontal='center')
            ...
            # Set column headers
            if evacuation_simulated:
                table_header(ws, row, col, ["EPZ Name", "Structure #", "PAR Day", "PAR Night", "LL Day", "LL Night", "Struc PreHaz", "Struc PostHaz", "PAR PreHaz (day)", "PAR PostHaz (day)", "LL PreHaz (day)", "LL PostHaz (day)", "% Warned", "% Mobilized", "Fatality Rate (day)", "LL Evac Day", "LL Evac Night"])
            else:
                table_header(ws, row, col, ["EPZ Name", "Structure #", "PAR Day", "PAR Night", "LL Day", "LL Night", "Struc PreHaz", "Struc PostHaz", "PAR PreHaz (day)", "PAR PostHaz (day)", "LL PreHaz (day)", "LL PostHaz (day)", "% Warned", "% Mobilized", "Fatality Rate (day)"])

            # excel Increment row for EPZ after the header
            current_row_epz = start_row_epz + 1
            current_row_total += 1

            # store mapping for later summary (reserve slot for convergence result)
            alt_to_sheet_map.append((alternative1, sanitized_sheet, hydscenario, haztime_formatted, epz_source, None, None, None))

            #create variables to track life loss where the time to no evac is before the earliest possible warning time
            total_lifeloss_noevac_beforewarn_day = 0
            total_lifeloss_noevac_beforewarn_night = 0

            ## LOOP B Begin looping through EPZ list, DistributionData table doesn't have named EPZ's just an order 0 to X
            epz_listorder = 0
            #accumulator for total life-loss per iteration across all EPZs
            total_iter_sums = None
            
            # Initialize accumulators for weighted correlation analysis across all EPZs
            all_epz_data_accumulated = []  # List of DataFrames from each EPZ
            all_epz_lifeloss = []  # Life loss for weighting each EPZ
            
            # Define field pairs for correlation analysis (used in both per-EPZ and weighted analyses)
            # Format: (predictor_field, outcome_field)
            correlation_pairs = [
                ("Relative_Warning_Issuance", "Total_Life_Loss"),
                ("Relative_Hazard_Identified", "Total_Life_Loss"),
                ("Communication_Delay", "Total_Life_Loss"),
                ("Warning_Issuance_Delay", "Total_Life_Loss"),
                ("Warning_Curve_Sample", "Total_Life_Loss"),
                ("Mobilization_Curve_Sample", "Total_Life_Loss")
            ]
            ...
            for epz in epz_list:
                ...
                ## BEGIN convergence test code
                # EPZ per-iteration samples table
                iteration_results = fr'"{simulation1}>EPZ_Samples_By_Iteration>{alternative1}>{detail_time}>{epz}"'
                df_area = pd.read_sql_query(f"SELECT * FROM {iteration_results}", con)

                # Run per-EPZ convergence test (keeps existing behavior)
                result_df = convergence_by_ci(df_area, DEFAULT_LL_COLUMNS, conf_level, conv_tol, min_iters)

                # Accumulate per-iteration totals for this EPZ into total_iter_sums
                try:
                    # sum the life-loss columns specified in DEFAULT_LL_COLUMNS per iteration (row-wise)
                    area_iter_totals = df_area[DEFAULT_LL_COLUMNS].sum(axis=1).astype(float).reset_index(drop=True)
                except Exception:
                    area_iter_totals = pd.Series(dtype=float)

                if total_iter_sums is None:
                    total_iter_sums = area_iter_totals.copy()
                else:
                    maxlen = max(len(total_iter_sums), len(area_iter_totals))
                    total_iter_sums = total_iter_sums.reindex(range(maxlen), fill_value=0)
                    area_iter_totals = area_iter_totals.reindex(range(maxlen), fill_value=0)
                    total_iter_sums = total_iter_sums + area_iter_totals

                
                # read convergence info from DataFrame attrs (convergence_by_ci stores info in attrs)
                converged = result_df.attrs.get("converged", False)
                converged_at = result_df.attrs.get("converged_at", None)
                final_error_pct = result_df.attrs.get("final_error_pct", None)
                conv_tol = result_df.attrs.get("conv_tol", None)
                conf_level = result_df.attrs.get("conf_level", None)
                final_error_pct_round = round(final_error_pct, 4) if final_error_pct is not None else None

                if converged == True:
                    add_message(f"...Converged at iteration: {converged_at} of {numiterations}", lst=2)
                    add_message(f"...Final convergence error: {final_error_pct_round}", lst=2)
                if converged != True:
                        add_message(f"...No convergence on {alternative1} - {epz}", lst=2)
                        add_message(f"...Final convergence error: {final_error_pct_round}", lst=2)
                        convergence_test_failed = True
                ## END convergence test code

                ## Determine correlation between Relative_Warning_Issuance value and Total_Life_Loss value for this EPZ to see if higher warning times are generally associated with lower life loss as expected. This is a check to see if the warning time is having the expected relationship with life loss outcomes, not a strict pass/fail test.
                
                # Define field pairs for correlation analysis
                # Format: (predictor_field, outcome_field)
                
                # Calculate correlations
                epz_correlations = calculate_correlations(df_area, correlation_pairs, add_message)
                
                # Create a sorted list of correlations by strength (absolute value) for ranking
                corr_strength_list = []
                for (field1, field2), results in epz_correlations.items():
                    if results["correlation"] is not None:
                        abs_corr = abs(results["correlation"]) if not np.isnan(results["correlation"]) else np.nan
                        corr_strength_list.append({
                            "field": field1,
                ...
                # Sort by absolute correlation strength (valid values first, NaN at bottom)
                corr_strength_list.sort(key=lambda x: (np.isnan(x["abs_r"]), -x["abs_r"] if not np.isnan(x["abs_r"]) else 0))
                
                # Display correlation ranking
                add_message("...Correlation Ranking with Total_Life_Loss (strongest to weakest):", lst=2)
                for rank, corr_item in enumerate(corr_strength_list, 1):
                    sig_indicator = "***" if corr_item["p_value"] < 0.05 else "ns"
                    add_message(f"...  {rank}. {corr_item['field']}: |r|={corr_item['abs_r']:.4f} (r={corr_item['r']:.4f}, p={corr_item['p_value']:.4f} {sig_indicator})", lst=2)
                

                # Accumulate data for weighted correlation analysis across all EPZs
                # Note: df_area contains data for all samples/iterations for this EPZ
                if len(df_area) > 0:
                    all_epz_data_accumulated.append(df_area.copy())
                    # Use epz_LL_day (which will be calculated below) as weight
                    # For now, we'll retrieve it here for accumulation
                    all_epz_lifeloss.append({"epz": epz, "lifeloss": None})  # Will update with actual LL value below

                # checks for epz type
                if any(sd in epz.lower() for sd in ("nb", "nf", "nonbreach", "non-breach", "non_breach", "nonfail", "non-fail", "non_fail", "nonfailure", "non-failure", "non_failure")):
                    epz_type = "NonFail"
                elif any(sd in epz.lower() for sd in ("inpool", "in-pool", "in_pool", "pool", "ip", "in pool")):
                    epz_type = "InPool"
                else:
                    epz_type = "Fail"
                add_message("Estimated EPZ type:   {}".format(epz_type), lst=2)
                ...
                if epz_warndistribution == "Uniform":
                    ...
                    if mmc_sop == True:
                        ...
                        if breach_condition == "NonFail" and epz_warndistribution_min != -3:
                            add_message("...MMC SOP Violation: This looks like a non-fail dam alternative, it shouldn't need a uniform distribution warning", lst=2)

                        # checks to see if the epz type is nonfail or in pool, and if so, it shouldn't have a unifrom warning distribution at all
                        if epz_type in ("NonFail", "InPool"):
                            add_message("...MMC SOP Violation: This looks like a non-fail or in-pool EPZ, it should be an early warning with no distribution", lst=2)
                ...
                epz_LL_struc_day_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE Emergency_Zone = ?"
                cursor.execute(epz_LL_struc_day_query, (epz,))
                epz_LL_struc_day = cursor.fetchone()[0] or 0

                epz_LL_evac_day_query = f"SELECT SUM(Life_Loss_Evacuating_Mean) FROM {results1_day_input} WHERE Emergency_Zone = ?"
                cursor.execute(epz_LL_evac_day_query, (epz,))
                epz_LL_evac_day = cursor.fetchone()[0] or 0

                epz_LL_day = epz_LL_struc_day + epz_LL_evac_day

                epz_LL_struc_night_query = f"SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE Emergency_Zone = ?"
                cursor.execute(epz_LL_struc_night_query, (epz,))
                epz_LL_struc_night = cursor.fetchone()[0] or 0

                epz_LL_evac_night_query = f"SELECT SUM(Life_Loss_Evacuating_Mean) FROM {results1_night_input} WHERE Emergency_Zone = ?"
                cursor.execute(epz_LL_evac_night_query, (epz,))
                epz_LL_evac_night = cursor.fetchone()[0] or 0

                epz_LL_night = epz_LL_struc_night + epz_LL_evac_night

                # Update the accumulated lifeloss tracker with the actual EPZ life loss value
                if all_epz_lifeloss and all_epz_lifeloss[-1]["epz"] == epz:
                    all_epz_lifeloss[-1]["lifeloss"] = epz_LL_day
                ...
                if epz_floodedbeforewarning:
                    ...
                    # count structures flooded in the EPZ before the earliest warning, but using time_to_no_evac instead of time_to_first_wet
                    epz_evacfloodedbeforewarning_query = f"SELECT COUNT(*) FROM {results1_day_input} WHERE Emergency_Zone = ? AND Time_To_No_Evac < {epz_earliestwarning_minutes} AND Max_Depth > 0"
                    cursor.execute(epz_evacfloodedbeforewarning_query, (epz,))
                    epz_evacfloodedbeforewarning = cursor.fetchone()[0]

                    # sum life loss day of those structures
                    epz_evacfloodedbeforewarning_LLday_query = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_day_input} WHERE Emergency_Zone = ? AND Time_To_No_Evac < {epz_earliestwarning_minutes} AND Max_Depth > 0"
                    cursor.execute(epz_evacfloodedbeforewarning_LLday_query, (epz,))
                    epz_evacfloodedbeforewarning_LLday_sum = cursor.fetchone()[0] or 0

                    epz_evacfloodedbeforewarning_LLnight_query = f"SELECT SUM(Life_Loss_Total_Mean) + SUM(Life_Loss_Evacuating_Mean) FROM {results1_night_input} WHERE Emergency_Zone = ? AND Time_To_No_Evac < {epz_earliestwarning_minutes} AND Max_Depth > 0"
                    cursor.execute(epz_evacfloodedbeforewarning_LLnight_query, (epz,))
                    epz_evacfloodedbeforewarning_LLnight_sum = cursor.fetchone()[0] or 0

                    total_lifeloss_noevac_beforewarn_day += epz_evacfloodedbeforewarning_LLday_sum
                    total_lifeloss_noevac_beforewarn_night += epz_evacfloodedbeforewarning_LLnight_sum

                    add_message("... POSSIBLE ISSUE:   {0}  structures with Life Loss of {1} (day) and {2} (night) have a Time_To_First_Wet before the earliest possible warning".format(epz_floodedbeforewarning, round(epz_floodedbeforewarning_LLday_sum,2), round(epz_floodedbeforewarning_LLnight_sum,2)), lst=2)
                    add_message("... HOWEVER:   {0}  structures with Life Loss of {1} (day) and {2} (night) have a Time_To_No_Evac before the earliest possible warning".format(epz_evacfloodedbeforewarning, round(epz_evacfloodedbeforewarning_LLday_sum,2), round(epz_evacfloodedbeforewarning_LLnight_sum,2)), lst=2)
                ...
                if evacuation_simulated:
                    ws.cell(row=current_row_epz, column=start_col_epz+15, value=epz_LL_evac_day or 0)
                    ws.cell(row=current_row_epz, column=start_col_epz+16, value=epz_LL_evac_night or 0)
                ...
                cell.number_format = '0.0%'  # Set the desired number format
                cell = ws.cell(row=current_row_epz, column=start_col_epz+15)
                cell.number_format = '#,##0.0'  # Set the desired number format
                cell = ws.cell(row=current_row_epz, column=start_col_epz+16)
                cell.number_format = '#,##0.0'  # Set the desired number format
            ...
            # Calculate weighted correlations across all EPZs

            add_message("-" * 80, lst=2)
            
            if all_epz_data_accumulated and len(all_epz_data_accumulated) > 0:
                # Extract weights (life loss values)
                weights_for_correlation = [item["lifeloss"] or 0 for item in all_epz_lifeloss]
                
                # Only proceed if we have non-zero weights
                if sum(weights_for_correlation) > 0:
                    # Normalize weights
                    total_weight = sum(weights_for_correlation)
                    normalized_weights = [w / total_weight for w in weights_for_correlation]
                    
                    #add_message("", lst=2)
                    #for i, (epz_info, weight) in enumerate(zip(all_epz_lifeloss, normalized_weights)):
                        #add_message(f"...EPZ '{epz_info['epz']}': Life Loss={epz_info['lifeloss']:.1f}, Weight={weight:.4f}", lst=2)
                    #add_message("", lst=2)
                    
                    # Calculate per-EPZ correlations and compute weighted average
                    ...
                    for field1, field2 in correlation_pairs:
                        ...
                        for idx, df_epz in enumerate(all_epz_data_accumulated):
                            if field1 not in df_epz.columns or field2 not in df_epz.columns:
                                continue
                            
                            # Extract valid pairs for this EPZ
                            valid_mask = df_epz[[field1, field2]].notna().all(axis=1)
                            valid_data = df_epz[valid_mask][[field1, field2]]
                            ...
                            if len(valid_data) >= 3:
                                try:
                                    ...
                                    if not np.isnan(corr_coef):
                                        ...
                                        valid_weights.append(normalized_weights[idx])
                                except Exception:
                                    pass
                        
                        # Compute weighted average of correlations
                        if valid_correlations:
                            # Re-normalize weights to account for filtered-out NaN values
                            valid_weight_sum = sum(valid_weights)
                            renormalized_weights = [w / valid_weight_sum for w in valid_weights]
                            weighted_avg_corr = np.average(valid_correlations, weights=renormalized_weights)
                            # For p-value, use unweighted average as conservative estimate
                            weighted_correlations[(field1, field2)] = {
                                "correlation": weighted_avg_corr,
                                "p_value": 0.0,  # Conservative: assume significant
                                "n": sum(len(df[[field1, field2]].dropna()) for df in all_epz_data_accumulated)
                            }
                        else:
                            weighted_correlations[(field1, field2)] = {
                                "correlation": None,
                    ...
                    # Create a sorted list of correlations by strength (absolute value) for ranking
                    weighted_corr_strength_list = []
                    for (field1, field2), results in weighted_correlations.items():
                        if results["correlation"] is not None:
                            abs_corr = abs(results["correlation"]) if not np.isnan(results["correlation"]) else np.nan
                            weighted_corr_strength_list.append({
                                "field": field1,
                                "r": results["correlation"],
                                "abs_r": abs_corr,
                                "p_value": results["p_value"],
                    ...
                    # Sort by absolute correlation strength (valid values first, NaN at bottom)
                    weighted_corr_strength_list.sort(key=lambda x: (np.isnan(x["abs_r"]), -x["abs_r"] if not np.isnan(x["abs_r"]) else 0))
                    
                    # Display weighted correlation ranking
                    add_message("TOTAL (all EPZs) Weighted Correlation Ranking (strongest to weakest):", lst=2)
                    add_message("     Pearson correlation coefficient r ranges from 0 (no correlation) to 1 (perfect correlation)", lst=2)
                    for rank, corr_item in enumerate(weighted_corr_strength_list, 1):
                        sig_indicator = "***" if corr_item["p_value"] < 0.05 else "ns"
                        add_message(f"  {rank}. {corr_item['field']}: |r|={corr_item['abs_r']:.4f} (r={corr_item['r']:.4f}, p={corr_item['p_value']:.4f} {sig_indicator})", lst=2)
                    
                    add_message("", lst=2)
                else:
                    add_message("No life loss data available for weighted correlation analysis", lst=2)
            else:
                add_message("Insufficient EPZ data for weighted correlation analysis", lst=2)

            # After iterating all EPZs for this alternative, run convergence on the total across EPZs
            if total_iter_sums is not None and len(total_iter_sums) > 0:
                df_total = pd.DataFrame({"Total_Life_Loss": total_iter_sums})
                total_result_df = convergence_by_ci(df_total, ["Total_Life_Loss"], conf_level, conv_tol, min_iters)
                total_converged = total_result_df.attrs.get("converged", False)
                total_converged_at = total_result_df.attrs.get("converged_at", None)
                total_final_error_pct = total_result_df.attrs.get("final_error_pct", None)
                total_final_error_pct_round = round(total_final_error_pct, 4) if total_final_error_pct is not None else None
                if total_converged:
                    add_message(f"TOTAL (all EPZs) Converged at iteration: {total_converged_at} of {numiterations}", lst=2)
                    add_message(f"TOTAL final convergence error: {total_final_error_pct_round}", lst=2)
                else:
                    add_message(f"POSSIBLE ISSUE - TOTAL (all EPZs) did NOT converge for {alternative1}", lst=2)
                    add_message(f"TOTAL final convergence error: {total_final_error_pct_round}", lst=2)
                    convergence_test_failed = True
                add_message("", lst=2)

            # Update the mapping entry with the actual convergence result
            try:
                alt_to_sheet_map[-1] = (alternative1, sanitized_sheet, hydscenario, haztime_formatted, epz_source, bool(total_converged), total_converged_at, numiterations)
            except Exception:
                # ensure mapping length mismatch won't crash final summary
                pass
            ...
            for summarypolygonset in cleaned_list:
                ...
                # sort the area_list alphabetically (case-insensitive) before area loops
                
                if alphabetize_areas:
                    area_list.sort(key=lambda s: s.lower())
                ...
                if evacuation_simulated:
                    if collapsed_column_exists:
                        table_header(ws, row, col, ["Area Name", "Structure #", "PAR Day", "PAR Night", "LL Day", "LL Night", "Collapsed >50pct", "Depth Range", "Velocity Range", "Arrival Range", "DxV Range", "Fatality Rate Day", "% Warned Day", "% Mobilized Day", "Struc PostHaz", "Avg Arrival PostHaz (hrs)", "Avg Flood Depth (ft)", "Arrival at first flooded PostHaz (hrs)", "LL Evac Day", "LL Evac Night"])
                    else:
                        table_header(ws, row, col, ["Area Name", "Structure #", "PAR Day", "PAR Night", "LL Day", "LL Night", "Depth Range", "Velocity Range", "Arrival Range", "DxV Range", "Fatality Rate Day", "% Warned Day", "% Mobilized Day", "Struc PostHaz", "Avg Arrival PostHaz (hrs)", "Avg Flood Depth (ft)", "Arrival at first flooded PostHaz (hrs)", "LL Evac Day", "LL Evac Night"])
                else:
                    if collapsed_column_exists:
                        table_header(ws, row, col, ["Area Name", "Structure #", "PAR Day", "PAR Night", "LL Day", "LL Night", "Collapsed >50pct", "Depth Range", "Velocity Range", "Arrival Range", "DxV Range", "Fatality Rate Day", "% Warned Day", "% Mobilized Day", "Struc PostHaz", "Avg Arrival PostHaz (hrs)", "Avg Flood Depth (ft)", "Arrival at first flooded PostHaz (hrs)"])
                    else:
                        table_header(ws, row, col, ["Area Name", "Structure #", "PAR Day", "PAR Night", "LL Day", "LL Night", "Depth Range", "Velocity Range", "Arrival Range", "DxV Range", "Fatality Rate Day", "% Warned Day", "% Mobilized Day", "Struc PostHaz", "Avg Arrival PostHaz (hrs)", "Avg Flood Depth (ft)", "Arrival at first flooded PostHaz (hrs)"])
                # excel Increment row for EPZ after the header
                ...
                for area in area_list:
                    ...
                    area_avgLL_struc_day_query = f'SELECT SUM(Life_Loss_Total_Mean) FROM {results1_day_input} WHERE "Summary_Set_{areapolygon}" = ?'
                    cursor.execute(area_avgLL_struc_day_query, (area,))
                    area_avgLL_struc_day = cursor.fetchone()[0] or 0

                    area_avgLL_evac_day_query = f'SELECT SUM(Life_Loss_Evacuating_Mean) FROM {results1_day_input} WHERE "Summary_Set_{areapolygon}" = ?'
                    cursor.execute(area_avgLL_evac_day_query, (area,))
                    area_avgLL_evac_day = cursor.fetchone()[0] or 0
                    #add_message("...Avg LL (day) in: {0} = {1}".format(area, round(area_avgLL_day,2)), lst=2)

                    area_avgLL_day = area_avgLL_struc_day + area_avgLL_evac_day
                    ...
                    area_avgLL_struc_night_query = f'SELECT SUM(Life_Loss_Total_Mean) FROM {results1_night_input} WHERE "Summary_Set_{areapolygon}" = ?'
                    cursor.execute(area_avgLL_struc_night_query, (area,))
                    area_avgLL_struc_night = cursor.fetchone()[0] or 0

                    area_avgLL_evac_night_query = f'SELECT SUM(Life_Loss_Evacuating_Mean) FROM {results1_night_input} WHERE "Summary_Set_{areapolygon}" = ?'
                    cursor.execute(area_avgLL_evac_night_query, (area,))
                    area_avgLL_evac_night = cursor.fetchone()[0] or 0

                    area_avgLL_night = area_avgLL_struc_night + area_avgLL_evac_night
                    ...
                    if collapsed_column_exists:
                        ws.cell(row=current_row_area, column=start_col_area+6, value=area_collapsed50p)
                    cell = ws.cell(row=current_row_area, column=start_col_area+7-rowmod_collapse, value=area_depth_range)
                    cell.alignment = Alignment(horizontal='center')
                    cell = ws.cell(row=current_row_area, column=start_col_area+8-rowmod_collapse, value=area_velocity_range)
                    cell.alignment = Alignment(horizontal='center')
                    cell = ws.cell(row=current_row_area, column=start_col_area+9-rowmod_collapse, value=area_arrival_day_range)
                    cell.alignment = Alignment(horizontal='center')
                    cell = ws.cell(row=current_row_area, column=start_col_area+10-rowmod_collapse, value=area_dxv_range)
                    cell.alignment = Alignment(horizontal='center')
                    ws.cell(row=current_row_area, column=start_col_area+11-rowmod_collapse, value=area_meanfatalityrate_day)
                    ws.cell(row=current_row_area, column=start_col_area+12-rowmod_collapse, value=area_par_warned_day_percentage)
                    ws.cell(row=current_row_area, column=start_col_area+13-rowmod_collapse, value=area_par_mobilized_day_percentage)
                    ws.cell(row=current_row_area, column=start_col_area+14-rowmod_collapse, value=area_posthazstructurecount_day)
                    ws.cell(row=current_row_area, column=start_col_area+15-rowmod_collapse, value=area_avgposthazardarrival_day_hrs)
                    ws.cell(row=current_row_area, column=start_col_area+16-rowmod_collapse, value=round(area_avgdepth_day, 1))
                    ws.cell(row=current_row_area, column=start_col_area+17-rowmod_collapse, value=area_first_flooded_posthazardarrival_day_hrs)
                    if evacuation_simulated:
                        ws.cell(row=current_row_area, column=start_col_area+18-rowmod_collapse, value=area_avgLL_evac_day)
                        ws.cell(row=current_row_area, column=start_col_area+19-rowmod_collapse, value=area_avgLL_evac_night)

                    # Apply number formatting to the relevant cells
                    for col_area in range(start_col_area+4, start_col_area+20):  # Adjust range as necessary, needs to be one greater than above table for some reason
                        cell = ws.cell(row=current_row_area, column=col_area)
                        cell.number_format = '#,##0.0'  # Set the desired number format
                    
                    for col_area2 in range(start_col_area+1, start_col_area+3):  # Adjust range as necessary, needs to be one greater than above table for some reason
                        cell = ws.cell(row=current_row_area, column=col_area2)
                        cell.number_format = '#,##0'  # Set the desired number format

                    #set mean fatality to 3 decimal places
                    cell = ws.cell(row=current_row_area, column=start_col_area+11-rowmod_collapse)
                    cell.number_format = '0.000'  # Set the desired number format

                    #set Percent Warned to 2 decimal places
                    cell = ws.cell(row=current_row_area, column=start_col_area+12-rowmod_collapse)
                    cell.number_format = '0.0%'  # Set the desired number format
                    
                    #set Percent Mobilized to 2 decimal places
                    cell = ws.cell(row=current_row_area, column=start_col_area+13-rowmod_collapse)
                    cell.number_format = '0.0%'  # Set the desired number format
            ...
            #report some EPZ totals
            add_message("", lst=2)
            add_message("Total Life Loss in structures with Time_To_No_Evac before earliest warning: {0} day... {1} night.".format(round(total_lifeloss_noevac_beforewarn_day,2), round(total_lifeloss_noevac_beforewarn_night,2)), lst=2)
            ## Begin looking at summary areas in the summary results
                                        
            # After EPZ and area tables, write top-10 structures by life loss for this alternative
            add_message("Writing Top-10 Structures by Life Loss...", lst=2)
            add_message("Note: fid values are not always the same as rowid numbers, and can be different in results files versus source files.", lst=2)
            time_table_list = [("Day", results1_day_input, total_lifeloss_day),("Night", results1_night_input, total_lifeloss_night)]
            for time_label, results_table, total_lifeloss in time_table_list:
                try:
                    top10_query = f"""
                        SELECT FID,
                            COALESCE(Occupancy_Type, '') AS Occupancy_Type,
                            COALESCE(Number_of_Stories, '') AS Number_of_Stories,
                            COALESCE(Emergency_Zone, '') AS Emergency_Zone,
                            COALESCE({first_summary_set}, '') AS Summary_Zone,
                            COALESCE(Collapsed, '') AS Collapsed,
                            (COALESCE(Pop_Under65_Mean,0) + COALESCE(PAR_Over65_Mean,0)) AS Total_PAR,
                            Max_Depth,
                            Max_Velocity,
                            Time_to_First_Wet,
                            COALESCE(Life_Loss_Evacuating_Mean,0) AS Life_Loss_Evac,
                            (COALESCE(Life_Loss_Total_Mean,0) + COALESCE(Life_Loss_Evacuating_Mean,0)) AS Total_Life_Loss
                        FROM {results_table}
                        ORDER BY Total_Life_Loss DESC
                        LIMIT 10
                    """
                    cursor.execute(top10_query)
                    top10_rows = cursor.fetchall()
                except sqlite3.OperationalError:
                    # fallback to minimum fields if some columns missing
                    try:
                        top10_query = f"""
                            SELECT FID,
                                Max_Depth,
                        ...
                        top10_rows = []
                #add_message(f"Rows: {top10_rows}", lst=2)
                # Write Top-10 block to the alternative worksheet
                top10_start_row = current_row_total + 2
                cell = ws.cell(row=top10_start_row, column=start_col_total, value=f"Structures: Top 10 by Life Loss ({time_label})")
                cell.font = Font(bold=True)
                header_row = top10_start_row + 1
                # determine if we have full columns (FID, Occupancy_Type, Number Stories, emergency_zone, summary zone, collapsed, Total_PAR, Max_Depth, Total_Life_Loss)
                #add_message(f"Rows: {len(top10_rows[0])}", lst=2)
                if top10_rows and len(top10_rows[0]) == 12:
                    headers = ["Rank", "Result FID", "Occupancy Type", "Number Stories", "EPZ", "Summary Area", "Collapsed", "PAR", "Max Depth", "Max Velocity", "Arrival Time hrs", "LL Evac", "Total LL", "% of Total LL"]
                else:
                    headers = ["Rank", "Result FID", "Max Depth", "Total Life Loss", "% of Total LL"]
                for c, h in enumerate(headers):
                    cell = ws.cell(row=header_row, column=start_col_total + c, value=h)
                    cell.alignment = Alignment(horizontal='center')
                    cell.font = Font(bold=True)
                # populate rows
                for i, rowvals in enumerate(top10_rows, start=1):
                    out_row = header_row + i
                    ws.cell(row=out_row, column=start_col_total, value=i)  # Rank
                    # handle both schemas
                    #add_message(f"Length of rowvals: {len(rowvals)}", lst=2)
                    if len(rowvals) == 12:
                        fid, occ, stories, epzname, summary_area, collapsed, total_par, depth, velocity, arrivaltime, ll_evac, total_ll= rowvals
                        cell = ws.cell(row=out_row, column=start_col_total+1, value=fid)
                        cell = ws.cell(row=out_row, column=start_col_total+2, value=occ)
                        cell.alignment = Alignment(horizontal='center')
                        cell = ws.cell(row=out_row, column=start_col_total+3, value=stories)
                        cell = ws.cell(row=out_row, column=start_col_total+4, value=epzname)
                        cell.alignment = Alignment(horizontal='center')
                        cell = ws.cell(row=out_row, column=start_col_total+5, value=summary_area)
                        cell.alignment = Alignment(horizontal='center')
                        cell = ws.cell(row=out_row, column=start_col_total+6, value=collapsed)
                        cell = ws.cell(row=out_row, column=start_col_total+7, value=total_par)
                        cell.number_format = '#,##0.0'
                        cell = ws.cell(row=out_row, column=start_col_total+8, value=depth)
                        cell.number_format = '#,##0.0'
                        cell = ws.cell(row=out_row, column=start_col_total+9, value=velocity)
                        cell.number_format = '#,##0.0'
                        cell = ws.cell(row=out_row, column=start_col_total+10, value=((arrivaltime - BreachTime_in_minutes)/60) if arrivaltime < 10000000 else 0)
                        cell.number_format = '#,##0.00'
                        ll_evac_cell = ws.cell(row=out_row, column=start_col_total+11, value=ll_evac or 0)
                        ll_evac_cell.number_format = '#,##0.0'
                        ll_cell = ws.cell(row=out_row, column=start_col_total+12, value=total_ll)
                        ll_cell.number_format = '#,##0.0'
                        # percent of total life loss (guard divide by zero)
                        pct = (total_ll / total_lifeloss) if (total_lifeloss and total_lifeloss > 0) else 0
                        cell = ws.cell(row=out_row, column=start_col_total+13, value=pct)
                        cell.number_format = '0.0%'
                        if pct > 0.10:
                            add_message(f"POSSIBLE ISSUE: Structure FID  {fid}  accounts for  {pct:.1%}  of total life loss in alternative {alternative1}", lst=2)
                    elif len(rowvals) == 3:
                        fid, depth, total_ll = rowvals
                        ws.cell(row=out_row, column=start_col_total+1, value=fid)
                        depth_cell = ws.cell(row=out_row, column=start_col_total+2, value=depth)
                        depth_cell.number_format = '#,##0.0'
                        ll_cell = ws.cell(row=out_row, column=start_col_total+3, value=total_ll)
                        ll_cell.number_format = '#,##0.0'
                        pct = (total_ll / total_lifeloss) if (total_lifeloss and total_lifeloss > 0) else 0
                        pct_cell = ws.cell(row=out_row, column=start_col_total+4, value=pct)
                        pct_cell.number_format = '0.0%'
                        if pct > 0.10:
                            add_message(f"POSSIBLE ISSUE: Structure FID  {fid}  accounts for  {pct:.1%}  of total life loss in alternative {alternative1}", lst=2)
                    else:
                        # unknown row shape, dump raw values then attempt percent if last value numeric
                        for j, v in enumerate(rowvals):
                            ws.cell(row=out_row, column=start_col_total+1+j, value=v)
                        try:
                            total_ll_guess = float(rowvals[-1])
                            pct = (total_ll_guess / total_lifeloss) if (total_lifeloss and total_lifeloss > 0) else 0
                            cell = ws.cell(row=out_row, column=start_col_total+1+len(rowvals), value=pct)
                            cell.number_format = '0.0%'
                            if pct > 0.10:
                                fid_guess = rowvals[0] if len(rowvals) > 0 else 'unknown'
                                add_message(f"POSSIBLE ISSUE: Structure FID  {fid_guess}  accounts for  {pct:.1%}  of total life loss in alternative {alternative1}", lst=2)
                        except Exception:
                            pass
                # advance current_row_total so following content doesn't overlap
                current_row_total = (header_row + len(top10_rows) + 1)

            if evacuation_simulated:
                # After EPZ and area tables, write top-10 Roads by life loss for this alternative
                add_message("Writing Top-10 Roads by Life Loss...", lst=2)
                time_table_roads_list = [("Day", results1_day_roads, total_lifeloss_evac_day, road_network),("Night", results1_night_roads, total_lifeloss_evac_night, road_network)]
                for time_label, results_table, total_road_lifeloss, road_network_name in time_table_roads_list:
                    try:
                        top10_query = f"""
                            SELECT ROWID,
                                COALESCE(CFCC, '') AS CFCC,
                                COALESCE(One_Way, '') AS One_Way,
                                COALESCE(Vertical_Offset, '') AS Vertical_Offset,
                                COALESCE(Max_Depth_ft, '') AS Max_Depth_ft,
                                COALESCE("Max_Velocity_ft/s", '') AS Max_Velocity,
                                COALESCE(Time_To_First_Wet, '') AS Arrival_Time,
                                COALESCE(Life_Loss_Mean,0) AS Life_Loss_Mean
                            FROM {results_table}
                            ORDER BY Life_Loss_Mean DESC
                            LIMIT 10
                        """
                        cursor.execute(top10_query)
                        top10_road_rows = cursor.fetchall()
                        #add_message(f"Top 10 road rows fetched: {top10_road_rows}", lst=2)
                    except sqlite3.OperationalError:
                        # fallback to minimum fields if some columns missing
                        try:
                            top10_query = f"""
                            ...
                            top10_road_rows = []
                    #pull road info data in
                    road_info_map = {}
                    try:
                        for row in top10_road_rows:
                            fid = row[0]
                            road_query = f"""
                                SELECT highway, name, "tiger:county", bridge, fid
                                FROM "{road_network_name}"
                                WHERE ROWID = ?
                            """
                            cursor.execute(road_query, (fid,))
                            info = cursor.fetchone()
                            if info:
                                highway, road_name, county, bridge, srfid = info
                            else:
                                highway = road_name = county = bridge = ''
                            # normalize None to empty string
                            road_info_map[fid] = (road_name or '', highway or '', county or '', bridge or '', srfid or '')
                            #add_message(f"Road network query FID {fid}: {(highway, road_name, county)}", lst=2)
                    except sqlite3.OperationalError:
                        add_message(f"Road network query failed", lst=2)
                    
                    #add_message(f"Rows: {top10_rows}", lst=2)
                    # Write Top-10 block to the alternative worksheet
                    top10road_start_row = current_row_total + 2
                    cell = ws.cell(row=top10road_start_row, column=start_col_total, value=f"Roads: Top 10 by Life Loss ({time_label})")
                    cell.font = Font(bold=True)
                    header_row = top10road_start_row + 1
                    # determine if we have full columns plus rank column ("FID", "CFCC", "One Way", "Vert Offset", "Max Depth", "Max Velocity", "Life Loss")
                    #add_message(f"Road Column: {len(top10_road_rows[0])}", lst=2)
                    if top10_road_rows and len(top10_road_rows[0]) == 8:
                        headers = ["Rank", "Result FID", "CFCC", "One Way", "Vert Offset", "Max Depth", "Max Velocity", "Arrival Time hrs", "Life Loss", "% of Road LL", "Road Name", "Highway Type", "County", "Bridge", "Source fid"]
                    else:
                        headers = ["Rank", "Result FID", "CFCC", "Life Loss", "% of Road LL", "Road Name", "Highway Type", "County"]
                    for c, h in enumerate(headers):
                        cell = ws.cell(row=header_row, column=start_col_total + c, value=h)
                        cell.alignment = Alignment(horizontal='center')
                        cell.font = Font(bold=True)
                    # populate rows
                    for i, rowvals in enumerate(top10_road_rows, start=1):
                        out_row = header_row + i
                        ws.cell(row=out_row, column=start_col_total, value=i)  # Rank
                        # handle both schemas
                        if len(rowvals) == 8:
                            fid, cfcc, oneway, vertoffset, maxdepth, maxveloc, arrival_time_road, total_ll = rowvals
                            # lookup road info we collected earlier
                            road_name, highway_type, county, bridge, srfid = road_info_map.get(fid, ('', '', '', '', ''))
                            cell = ws.cell(row=out_row, column=start_col_total+1, value=fid)
                            cell = ws.cell(row=out_row, column=start_col_total+2, value=cfcc)
                            cell.alignment = Alignment(horizontal='center')
                            cell = ws.cell(row=out_row, column=start_col_total+3, value=oneway)
                            cell.alignment = Alignment(horizontal='center')
                            cell = ws.cell(row=out_row, column=start_col_total+4, value=vertoffset)
                            cell.alignment = Alignment(horizontal='center')
                            cell = ws.cell(row=out_row, column=start_col_total+5, value=maxdepth)
                            cell.number_format = '#,##0.0'
                            cell = ws.cell(row=out_row, column=start_col_total+6, value=maxveloc)
                            cell.number_format = '#,##0.0'
                            cell = ws.cell(row=out_row, column=start_col_total+7, value=((arrival_time_road - BreachTime_in_minutes)/60) if arrival_time_road else 0)
                            cell.number_format = '#,##0.00'
                            cell = ws.cell(row=out_row, column=start_col_total+8, value=total_ll)
                            cell.number_format = '#,##0.0'
                            # percent of total life loss (guard divide by zero)
                            pct = (total_ll / total_road_lifeloss) if (total_road_lifeloss and total_road_lifeloss > 0) else 0
                            cell = ws.cell(row=out_row, column=start_col_total+9, value=pct)
                            cell.number_format = '0.0%'
                            ws.cell(row=out_row, column=start_col_total+10, value=road_name)
                            ws.cell(row=out_row, column=start_col_total+11, value=highway_type)
                            ws.cell(row=out_row, column=start_col_total+12, value=county)
                            ws.cell(row=out_row, column=start_col_total+13, value=bridge)
                            ws.cell(row=out_row, column=start_col_total+14, value=srfid)
                            if pct > 0.10:
                                add_message(f"POSSIBLE ISSUE: Road FID  {fid}  accounts for  {pct:.1%}  of total road life loss in alternative {alternative1}", lst=2)
                        elif len(rowvals) == 3:
                            fid, cfcc, total_ll = rowvals
                            ws.cell(row=out_row, column=start_col_total+1, value=fid)
                            cfcc_cell = ws.cell(row=out_row, column=start_col_total+2, value=cfcc)
                            cfcc_cell.number_format = '#,##0.0'
                            ll_cell = ws.cell(row=out_row, column=start_col_total+3, value=total_ll)
                            ll_cell.number_format = '#,##0.0'
                            pct = (total_ll / total_road_lifeloss) if (total_road_lifeloss and total_road_lifeloss > 0) else 0
                            pct_cell = ws.cell(row=out_row, column=start_col_total+4, value=pct)
                            ...
                            try:
                                total_ll_guess = float(rowvals[-1])
                                pct = (total_ll_guess / total_road_lifeloss) if (total_road_lifeloss and total_road_lifeloss > 0) else 0
                                cell = ws.cell(row=out_row, column=start_col_total+1+len(rowvals), value=pct)
                                cell.number_format = '0.0%'
                                if pct > 0.10:
                                    fid_guess = rowvals[0] if len(rowvals) > 0 else 'unknown'
                                    add_message(f"POSSIBLE ISSUE: Road FID  {fid_guess}  accounts for  {pct:.1%}  of total road life loss in alternative {alternative1}", lst=2)
                            except Exception:
                                pass
                    # advance current_row_total so following content doesn't overlap
                    current_row_total = (header_row + len(top10_road_rows) + 1)
            ...
            for idx, message in enumerate(message_list2, start=1):
                ...
                if "POSSIBLE ISSUE" in message.upper().strip():
                    ...
                    cleaned_message = re.sub(r'(?i)^\.*\s*POSSIBLE ISSUE:\s*', '', message).strip()
                    add_message("{0}:  {1}".format(alternative1, cleaned_message), lst=1)
                ...
                if "MMC SOP VIOLATION" in message.upper().strip():
                    ...
                    add_message("{0}: MMC SOP VIOLATION- {1}".format(alternative1, message), lst=1)
                
                if "Converged: False" in message.upper().strip():
                    idxcell.font = orange_font  # Apply the orange font
        ...
        for inventory in inventorylist:
            ...
            for field, aggs in inventory_summary.items():
                add_message(f"{inventory} - {field}:    " +
                    "    ".join([
                        f"{agg}= {f'{aggs[agg]:,.1f}' if isinstance(aggs[agg], (int, float)) else aggs[agg]}"
                        for agg in aggs
                    ]),
                    lst=1
                )
            ...
            # flag if the maximum stories_number is greater than 100 (Willis Tower in Chicago has the most stories in US at 110)
            max_stories = inventory_summary.get("Stories_Number", {}).get("MAX", 0)
            if max_stories > 100:
                add_message(f"POSSIBLE ISSUE: Max number of stories in inventory '{inventory}' is {max_stories}, only 5 US buildings have 100-110 stories.", lst=1)
        ...
        # as of 3/3/2026 this appears to get triggered regardless of whether convergence test actually failed or not, so commenting out for now.
        #if convergence_test_failed:
            #add_message("Convergence test may have failed for one or more alternatives", lst=1)
        ...
        for idx1, message1 in enumerate(message_list1, start=1):
            ...
            if "Convergence test failed" in message1:
                cell1.font = red_font
        ...
        if alt_to_sheet_map:
            cell = ws.cell(row=mapping_start_row, column=1, value="Alternative Name")
            cell.alignment = Alignment(horizontal='center')
            cell.font = Font(bold=True)
            cell = ws.cell(row=mapping_start_row, column=2, value="Sanitized Sheet Name")
            cell.alignment = Alignment(horizontal='center')
            cell.font = Font(bold=True)
            cell = ws.cell(row=mapping_start_row, column=3, value="Hydraulic Scenario")
            cell.alignment = Alignment(horizontal='center')
            cell.font = Font(bold=True)
            cell = ws.cell(row=mapping_start_row, column=4, value="Hazard Time")
            cell.alignment = Alignment(horizontal='center')
            cell.font = Font(bold=True)
            cell = ws.cell(row=mapping_start_row, column=5, value="EPZ Source")
            cell.alignment = Alignment(horizontal='center')
            cell.font = Font(bold=True)
            cell = ws.cell(row=mapping_start_row, column=6, value="Converged At")
            cell.alignment = Alignment(horizontal='center')
            cell.font = Font(bold=True)

            for i, (altname, sheetname, hydscenario, haztime_formatted, epz_source, total_converged, total_converged_at, numiterations) in enumerate(alt_to_sheet_map, start=1):
                cell = ws.cell(row=mapping_start_row + i, column=1, value=altname)
                cell = ws.cell(row=mapping_start_row + i, column=2, value=sheetname)
                cell = ws.cell(row=mapping_start_row + i, column=3, value=hydscenario)
                cell = ws.cell(row=mapping_start_row + i, column=4, value=haztime_formatted)
                cell = ws.cell(row=mapping_start_row + i, column=5, value=epz_source)
                cell = ws.cell(row=mapping_start_row + i, column=6, value=f"{total_converged_at} of {numiterations}")
