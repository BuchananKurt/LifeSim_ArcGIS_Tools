    ...
    def execute(self, parameters, messages):
        ...
        for alternative1 in alternative_names_list:
            ...
            for epz in epz_list:
                ...
                iteration_results_day = fr'"{simulation1}>EPZ_Samples_By_Iteration>{alternative1}>14>{epz}"'
                df_area = pd.read_sql_query(f"SELECT * FROM {iteration_results_day}", con)
            ...
            for summarypolygonset in cleaned_list:
                ...
                for area in area_list:
                    ...
                    if evacuation_simulated:
                        ws.cell(row=current_row_area, column=start_col_area+18, value=area_avgLL_evac_day)
                        ws.cell(row=current_row_area, column=start_col_area+19, value=area_avgLL_evac_night)

                    # Apply number formatting to the relevant cells
                    for col_area in range(start_col_area+3, start_col_area+20):  # Adjust range as necessary, needs to be one greater than above table for some reason
        ...
        if convergence_test_failed:
            add_message("Convergence test failed for one or more alternatives", lst=1)
